...
# Version: 6.2 (Add Nodal Displacement Export based on User V3.4) # <<< 版本号更新
# Extracts Max Contact Stresses (Sheet1 from stress_node_set_name)
# and Nodal Displacements (Sheet2 from disp_node_set_name)
...
# --- 尝试导入 openpyxl 并初始化 Debug Excel (V6.2 严格结构) ---
try:
...
def log_to_excel(level, message, exc_obj=None):
    ...
    try:
        ...
        if exc_obj: # 独立行
            ...
            except Exception as format_exc_err: # 独立行
                try: # 尝试获取 repr
...
disp_node_set_name = 'NS_CONTACT_SLAVE'   # <<< 用于提取位移的节点集 (软体)
# !!! ----------------------------------- !!!
...
for i in range(num_iterations): # <<< 修改：使用 num_iterations
    ...
    try: # ODB Main Try
        ...
        if not odb_exists: # if 块开始
             ...
             # 记录错误并跳过此迭代的 ODB 处理
             node_set_region_stress = None
             node_set_region_disp = None
             last_frame = None
            ...
            disp_node_set_region = None   # 位移区域对象
