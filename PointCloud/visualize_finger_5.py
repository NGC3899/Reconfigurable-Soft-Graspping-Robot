...
translation_vector = np.array([15, 10, 25.5]) # <<< 调整球体/对象初始中心位置
...
# --- 辅助函数 ---
# generate_sphere_points 和 transform_points 函数同前，此处省略
def generate_sphere_points(radius, num_points):
    points = []; phi = np.pi * (3. - np.sqrt(5.))
    for i in range(num_points):
        y = 1 - (i / float(num_points - 1)) * 2; radius_at_y = np.sqrt(1 - y * y)
        theta = phi * i; x = np.cos(theta) * radius_at_y; z = np.sin(theta) * radius_at_y
        points.append((x, y, z))
    return np.array(points) * radius

def transform_points(points, matrix):
    if points.shape[1] != 3: raise ValueError("Input points should be N x 3 shape")
    if matrix.shape != (4, 4): raise ValueError("Transformation matrix should be 4 x 4 shape")
    num_pts = points.shape[0]; homogeneous_points = np.hstack((points, np.ones((num_pts, 1))))
    transformed_homogeneous = homogeneous_points @ matrix.T
    return transformed_homogeneous[:, :3] / transformed_homogeneous[:, 3, np.newaxis]
# --- 主代码逻辑 ---

# 读取和处理节点/Excel数据的代码同前，此处省略...
...
try:
    with open(node_list_file_path, 'r') as f:
        for line in f:
            ...
            if line_stripped:
                try: node_labels_from_file.append(int(line_stripped))
                except ValueError: pass
except Exception as e: print(f"读取节点列表文件时出错: {e}"); sys.exit()
try: df = pd.read_excel(excel_file_path, sheet_name=sheet_name, engine='openpyxl')
except Exception as e: print(f"读取 Excel 文件时出错: {e}"); sys.exit()
required_columns = ['Iteration', 'Pressure', 'NodeLabel', 'X0', 'Y0', 'Z0', 'U1', 'U2', 'U3']
if not all(col in df.columns for col in required_columns): print("错误：Excel 文件中缺少必需的列。"); sys.exit()
df_iter = df[df['Iteration'] == iteration_to_plot]
if df_iter.empty: print(f"错误：找不到迭代次数 {iteration_to_plot} 的数据。"); sys.exit()
...
if not nodes_to_process: print("错误：来自列表的节点在迭代数据中均未找到。"); sys.exit()
...
if len(df_relevant_nodes) > 1:
    ...
    start_label = nodes_to_process[0]; current_label = start_label
    spatially_sorted_labels.append(current_label); remaining_labels.remove(current_label)
    while remaining_labels:
        last_coord = node_coords_map[current_label]; min_dist_sq = float('inf'); nearest_label = -1
        for label in remaining_labels:
            dist_sq = np.sum((last_coord - node_coords_map[label])**2)
            if dist_sq < min_dist_sq: min_dist_sq = dist_sq; nearest_label = label
        if nearest_label != -1: spatially_sorted_labels.append(nearest_label); remaining_labels.remove(nearest_label); current_label = nearest_label
        else: break
    df_filtered_sorted = df_relevant_nodes.set_index('NodeLabel').loc[spatially_sorted_labels].reset_index()
else: df_filtered_sorted = df_relevant_nodes
if df_filtered_sorted.empty: print("错误：筛选和排序后没有剩余数据。"); sys.exit()
try:
...
except Exception as e_calc: print(f"计算变形坐标时出错: {e_calc}"); sys.exit()
curve1_points = df_filtered_sorted[['X_def', 'Y_def', 'Z_def']].values
if len(curve1_points) < 2: print("错误：需要至少两个点来定义手指表面。"); sys.exit()
curve2_points = curve1_points.copy(); curve2_points[:, 1] += y_translation
...
if finger_faces: faces_np = np.hstack(finger_faces)
else: faces_np = np.array([], dtype=int)
...
try:
    print("计算单元法线...")
    # auto_orient_normals 尝试使法线方向一致（例如指向外侧）
    finger_mesh.compute_normals(point_normals=False, cell_normals=True, inplace=True, auto_orient_normals=True)
    if 'Normals' not in finger_mesh.cell_data:
         raise RuntimeError("无法计算手指网格的单元法线。")
    using_cell_normals = True # 明确使用单元法线
    print("成功计算单元法线。")
...
contact_points_data = [] # 存储 (距离, 点索引, 相对位置符号 side_sign)
min_distance_overall = float('inf')
closest_point_index_overall = -1
overlap_detected = False
found_positive_side = False # 标记阈值内是否有在外侧的点
found_negative_side = False # 标记阈值内是否有在内侧的点
...
for i in range(num_object_points):
    ...
    try:
        # --- 使用 find_closest_cell 和 cell_normals ---
        closest_cell_id, closest_point_coords = finger_mesh.find_closest_cell(point, return_closest_point=True)
        if closest_cell_id < 0:
            # print(f"警告：点 {i} 未找到最近单元。") # 可选的详细警告
            continue # 未找到有效单元，跳过此点

        # 获取对应单元的法线
        normal = finger_mesh.cell_normals[closest_cell_id]
        distance = np.linalg.norm(point - closest_point_coords)
        # --- 结束单元法线逻辑 ---

        # 计算相对位置向量和符号
        vector_to_point = point - closest_point_coords
        if np.linalg.norm(vector_to_point) > 1e-9: # 避免除零或无效点积
            # 标准化向量可以使点积更稳定，但不是必须的，因为我们只需要符号
            # vector_to_point_normalized = vector_to_point / np.linalg.norm(vector_to_point)
            # normal_normalized = normal / np.linalg.norm(normal) # 法线通常是单位向量
            side_sign = np.sign(np.dot(vector_to_point, normal))
        else:
            side_sign = 0 # 点重合或极近

    except IndexError:
        print(f"警告：访问单元法线时索引错误 (Cell ID: {closest_cell_id})。可能网格或法线数据有问题。跳过点 {i}。")
        continue
    ...
    # 如果未重叠，检查是否在接触阈值内
        ...
        contact_points_data.append((distance, i, side_sign))
        # --- 关键：只有在阈值内的点才用来判断两侧分布 ---
        if side_sign > 0:
...
if overlap_detected:
    ...
    contact_indices_to_visualize = [data[1] for data in contact_points_data]
     ...
     # 只有当阈值内的点确实分布在两侧时，才判断为交叉
    final_state = "Intersection"
    print("最终状态：交叉穿过 (Intersection)")
    contact_indices_to_visualize = [data[1] for data in contact_points_data]
elif contact_points_data: # 仅在一侧有接触点 (found_positive_side XOR found_negative_side is True, or one is True and other is False)
    final_state = "Contact"
    print("最终状态：接触 (Contact)")
    contact_points_data.sort(key=lambda x: x[0])
    closest_distance, closest_contact_index, closest_side_sign = contact_points_data[0]
    ...
    print(f"最近接触点索引: {closest_contact_index}, 局部坐标: {closest_contact_local_coords}, 距离: {closest_distance:.4f}, 相对侧: {'外侧' if closest_side_sign > 0 else '内侧' if closest_side_sign < 0 else '表面'}")
    contact_indices_to_visualize = [data[1] for data in contact_points_data]
else:
    print("最终状态：无接触 (No Contact)")
    if closest_point_index_overall != -1: print(f"点云中离手指表面最近的点索引为 {closest_point_index_overall}, 全局坐标为 {object_points_global[closest_point_index_overall]}, 距离为 {min_distance_overall:.4f} (大于阈值)。")


# --- 使用 PyVista 进行可视化 (代码同上次，绘制逻辑基于 final_state) ---
print("正在绘制网格和点云...")
try: plotter = pv.Plotter(window_size=[1000, 800])
except Exception as e_plotter: print(f"初始化 PyVista Plotter 时出错: {e_plotter}"); sys.exit()

# 添加手指网格
try: plotter.add_mesh(finger_mesh, color='lightblue', style='surface', opacity=0.7, smooth_shading=True, label='Finger Surface')
except Exception as e_add_finger: print(f"添加手指网格到绘图器时出错: {e_add_finger}")

# 添加对象点云（使用上次的分开绘制逻辑）
try:
    num_object_points_viz = object_points_global.shape[0]
    non_contact_mask = np.ones(num_object_points_viz, dtype=bool)
    highlight_points_coords = np.empty((0, 3)) # 初始化为空
    ...
    if highlight_points_coords.shape[0] > 0:
        highlight_color = 'gray' # 默认灰色，理论上不应使用
        highlight_label = 'Highlight Points (Unknown State)'
        if final_state == "Intersection":
             highlight_color = intersection_point_color; highlight_label = 'Object Points (Intersection)'
        elif final_state == "Contact":
             highlight_color = colliding_point_color; highlight_label = f'Object Points (Contact, Dist < {collision_threshold})'
        elif final_state == "Overlap Detected":
             highlight_color = 'orange'; highlight_label = 'Object Points (Near Overlap)'
...
except Exception as e_add_points: print(f"添加对象点云到绘图器时出错: {e_add_points}")

# 可视化质心和坐标轴 (代码同前)
try:
    plotter.add_points(centroid, color='green', point_size=15, render_points_as_spheres=True, label='Object Centroid')
    x_axis_end = centroid + [axis_length, 0, 0]; x_axis = pv.Line(centroid, x_axis_end)
    plotter.add_mesh(x_axis, color='red', line_width=5, label='Object X-axis')
    y_axis_end = centroid + [0, axis_length, 0]; y_axis = pv.Line(centroid, y_axis_end)
    plotter.add_mesh(y_axis, color='green', line_width=5, label='Object Y-axis')
    z_axis_end = centroid + [0, 0, axis_length]; z_axis = pv.Line(centroid, z_axis_end)
    plotter.add_mesh(z_axis, color='blue', line_width=5, label='Object Z-axis')
except Exception as e_add_centroid_axes: print(f"添加质心或坐标轴时出错: {e_add_centroid_axes}")

# 添加标题和图例 (代码同前)
try:
    plotter.add_text(f"手指-对象点云状态: {final_state} (迭代: {iteration_to_plot})", position="upper_edge", font_size=12)
    plotter.add_legend()
    plotter.add_axes()
except Exception as e_add_extras: print(f"添加文本/图例/坐标轴时出错: {e_add_extras}")

# 显示绘图窗口
print("正在显示交互式 3D 绘图...")
try: plotter.show()
except Exception as e_show: print(f"显示绘图窗口时出错: {e_show}")
print("绘图窗口已关闭或显示失败。")