# -*- coding: utf-8 -*-
# Version: 6.1 (Add Nodal Displacement Export based on User V3.4)
...
# --- 尝试导入 openpyxl 并初始化 Debug Excel (V6.1 严格结构) ---
try:
...
# <<< 修改：现在只需要指定单个 CAE 文件 >>>
cae_file_path = r'D:\softgripper2.cae' # <<< 指定包含特定位置的 CAE 文件路径
model_name = 'Model-1'
# ... (pressure_load_names 不变) ...
pressure_load_names = ['pressure1', 'pressure2', 'pressure3', 'pressure4a', 'pressure4b', 'pressure4c', 'pressure4d', 'pressure4e', 'pressure4f', 'pressure5a', 'pressure5b', 'pressure5c', 'pressure5d', 'pressure5e']
num_iterations = 2 # <<< 仅压力迭代次数
...
log_to_excel("DEBUG", "Node Set for Stress: {}".format(stress_node_set_name)) # <<< 修改日志
log_to_excel("DEBUG", "Node Set for Displacement: {}".format(disp_node_set_name)) # <<< 新增日志
...
try:
    ...
    if myAssembly: # 检查 Assembly 对象是否存在
        ...
        # except 块开始 (MDB中找不到节点集)
        except KeyError as e_set:
        ...
        # except 块开始 (其他获取初始坐标错误)
        except Exception as e_get_init:
...
for i in range(num_iterations): # <<< 修改：使用 num_iterations
    ...
    try: # ODB Main Try
        ...
        if not odb_exists: # if 块开始
            ...
            node_set_region_disp = None   # <<< 修改：重命名以区分 ODB 对象 >>>
                        ...
                        if disp_subset and hasattr(disp_subset, 'values') and disp_subset.values: # 检查子集和 values 是否有效
                            ...
                            for fv in disp_subset.values: # for 循环开始
                                try: # try for processing one FieldValue
                                    ...
                                    # 获取位移数据 (U1, U2, U3) - 假设 data 是包含 3 个浮点数的序列
                                    if hasattr(fv, 'data') and isinstance(fv.data, (list, tuple)) and len(fv.data) >= 3: # if 块开始
                                        u1 = fv.data[0] # 独立行
                                        u2 = fv.data[1] # 独立行
                                        u3 = fv.data[2] # 独立行
                                        # 从 MDB 获取初始坐标
                                        ...
                                        else: # else 块开始 (ws_disp 无效)
                                            log_to_excel("ERROR", "Displacement sheet (ws_disp) is not available!") # 独立行
                                    else: # else 块开始 (fv.data 格式不对)
                                        log_to_excel("WARNING", "Displacement data for node {} has unexpected format or length: {}".format(node_label, fv.data)) # 独立行
                                ...
                                except (IndexError, TypeError) as e_fv_data: # except 块开始 (处理 data 索引或类型错误)
                                     log_to_excel("WARNING", "Error processing displacement data for node {}: {}".format(fv.nodeLabel if hasattr(fv, 'nodeLabel') else 'Unknown', fv.data), exc_obj=e_fv_data) # 独立行
...
log_to_excel("INFO", "Attempting to save final Result Excel workbook to: {}".format(output_excel_file)) # 独立行
try:
    if wb_result: # if 块开始
        ...
        log_to_excel("INFO", "All {} iterations appear completed.".format(num_iterations)) # <<< 使用 num_iterations
        ...
        log_to_excel("DEBUG", "Result Excel workbook saved successfully.") # 独立行
         ...
         log_to_excel("ERROR","Result workbook (wb_result) is not available.") # 独立行
except Exception as e: # except 块开始
    log_to_excel("ERROR", "Failed to save the Result Excel file '{}'...".format(output_excel_file), exc_obj=e) # 独立行
