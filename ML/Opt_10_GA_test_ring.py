# -*- coding: utf-8 -*-
# 本脚本使用遗传算法的方式进行构型推荐程序的目标函数求解，使用理想甜甜圈点云测试
import numpy as np
...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_GA_Torus - 使用遗传算法和程序化甜甜圈进行测试")
...
def create_torus_point_cloud(ring_radius, tube_radius, center, n_points=500):
...
if __name__ == '__main__':
    ...
    for gen in range(GA_NUM_GENERATIONS):
        ...
        for i, chromosome in enumerate(population):
            ...
            if fitness > best_overall_gii:
                ...
                print(f"  ** 新的最优解! Gen {gen+1}, Ind {i+1}, GII = {-cost:.4f}, r={r_val:.3f}, pos={pos_indices} **")

        # 3. 选择
        # 精英保留
        sorted_evaluated_population = sorted(evaluated_population_details, key=lambda x: x["fitness"], reverse=True)
        new_population = [list(ind["chromosome"]) for ind in sorted_evaluated_population[:GA_ELITISM_COUNT]]

        # 轮盘赌选择剩余个体
        selectable_population = sorted_evaluated_population[GA_ELITISM_COUNT:]
        if selectable_population: # 仅在有非精英个体时进行轮盘赌
            min_fitness = min(ind["fitness"] for ind in selectable_population)
            adjusted_fitness_values = [ind["fitness"] - min_fitness + 1e-6 for ind in selectable_population]
            total_adjusted_fitness = sum(adjusted_fitness_values)

            for _ in range(GA_POPULATION_SIZE - GA_ELITISM_COUNT):
                if total_adjusted_fitness <= 0:
                    pick_details = random.choice(selectable_population)
                else:
                    r_pick = random.uniform(0, total_adjusted_fitness)
                    current_sum = 0
                    for i, ind_details in enumerate(selectable_population):
                        current_sum += adjusted_fitness_values[i]
                        if current_sum > r_pick:
                            pick_details = ind_details
                            break
                    else:
                        pick_details = random.choice(selectable_population) # Fallback
        ...
        # 从精英之外选择配对进行交叉
        mating_pool = population[GA_ELITISM_COUNT:]
        random.shuffle(mating_pool)

        for i in range(0, len(mating_pool), 2):
            if i + 1 >= len(mating_pool): # 如果是奇数，最后一个直接加入
                offspring_population.append(mating_pool[i])
                continue
            ...
            for child in [child1, child2]:
                if random.random() < GA_MUT_PROB:
                    # r 基因变异
                    child[0] += random.gauss(0, (R_BOUNDS[1] - R_BOUNDS[0]) * 0.1)
                    child[0] = np.clip(child[0], R_BOUNDS[0], R_BOUNDS[1])
                if random.random() < GA_MUT_PROB:
                    # combo_idx 基因变异
                    child[1] = random.randint(0, num_valid_combinations - 1)
        ...
        current_best_fitness_gen = max(fitness_values)
        print(f"  代 {gen + 1} 最佳成本: {-current_best_fitness_gen:.4f} (GII: {-current_best_fitness_gen:.4f} if valid)")
        print(f"  迄今为止最优成本: {-best_overall_gii:.4f} (GII: {best_overall_gii:.4f})")
    ...
    if best_overall_chromosome is not None:
        ...
        print(f"  pos_indices= {best_pos_indices_ga}")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (GA - Torus)", off_screen=False)
            ...
            params_txt_ga=f"r={best_r_ga:.2f}, P={pressures_display_str}, indices={final_chosen_indices_ga_viz}"
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (GA - Torus)"
            )
        else:
