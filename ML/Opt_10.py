...
# --- 打印版本信息 ---
try:
    print(f"Opt_9_modified.txt (PyVista Interaction & O3D Tray Wireframe) - OBJECT LONGEST EDGE ALIGNED TO LOCAL X, N_SLOTS FINGER POSITIONS")
...
NUM_INITIAL_SEARCHES = 5 # Reduced for quicker testing with new logic, can be increased
PRESSURE_STEP_INIT_SEARCH = 500.0
N_CALLS_BO = 200        # Reduced for quicker testing
N_INITIAL_POINTS_BO = 2 # Reduced for quicker testing
R_BOUNDS = (tray_radius * 0.3, tray_radius * 0.95)
P_BOUNDS = (0.0, max_pressure)
# ANGLE_OFFSET_BOUNDS = (0.0, 120.0) # Removed
N_FINGER_SLOTS = 9 # New parameter n
...
param_names = ['r', 'p1', 'p2', 'p3', 'pos_idx1', 'pos_idx2', 'pos_idx3'] # Modified
dimensions = [
    ...
    Integer(name='pos_idx1', low=0, high=N_FINGER_SLOTS - 1), # New
    Integer(name='pos_idx2', low=0, high=N_FINGER_SLOTS - 1), # New
    Integer(name='pos_idx3', low=0, high=N_FINGER_SLOTS - 1)  # New
]
...
def get_orthogonal_vectors(normal_vector):
    ...
    if norm_t1 < 1e-9:
        ...
        if norm_t1 < 1e-9:
            ...
            else: t1 = np.array([1.,0.,0.]) # Fallback if n is parallel to z and was already parallel to y
            norm_t1 = LA.norm(t1)
            if norm_t1 < 1e-9: raise ValueError("Fallback t1 is zero.") # Should not happen if n is not zero
    t1 /= norm_t1
...
def calculate_gii_multi_contact(contacts_info,object_centroid_for_gii,mu,eigenvalue_thresh):
    ...
    for contact in contacts_info:
        ...
        n_contact = - (normal_finger / norm_mag) # Normal pointing from finger into object
        try:
            t1, t2 = get_orthogonal_vectors(n_contact)
        except ValueError:
            continue
        r_contact = pt_on_mesh - object_centroid_for_gii
        # Friction cone approximated by 4 vectors
    ...
    if valid_contacts_for_gii < 2 : return None # Need at least two valid contacts for GII
    grasp_matrix_G = np.column_stack(all_wrenches)
    J = grasp_matrix_G @ grasp_matrix_G.T # 6x6 matrix
    try:
        eigenvalues = LA.eigvalsh(J) # Use eigvalsh for symmetric matrices
        ...
        if len(non_zero_eigenvalues) > 0:
            ...
            # Allow very small negative lambda_min if close to zero (numerical precision)
            if lambda_min < 0 and np.abs(lambda_min) < eigenvalue_thresh: lambda_min = 0.0
            elif lambda_min < 0: return None # Should not happen for J = G G^T if G is real
            lambda_max = np.max(non_zero_eigenvalues)
            if lambda_max < eigenvalue_thresh: return 0.0 # All significant eigenvalues are too small
            return np.sqrt(lambda_min / lambda_max) if lambda_max > 1e-9 else 0.0
        else:
            return 0.0 # No significant positive eigenvalues
...
def evaluate_grasp(r, p1, p2, p3, pos_idx1, pos_idx2, pos_idx3): # MODIFIED SIGNATURE
    ...
    if len(set(chosen_indices)) < 3:
        # Penalize if indices are not distinct, as this means fingers are overlapping at placement
        # print(f"  评估: r={r:.4f}, P=({p1:.1f},{p2:.1f},{p3:.1f}), pos=({pos_idx1},{pos_idx2},{pos_idx3}) -> Indices not distinct, Cost=10.0")
        return 10.0

    print_eval_string = f"  评估: r={r:.4f}, p1={p1:.1f}, p2={p2:.1f}, p3={p3:.1f}, pos=({chosen_indices[0]},{chosen_indices[1]},{chosen_indices[2]})" # MODIFIED
    ...
    for i in range(3): # For each of the 3 fingers
        ...
        # MODIFIED FINGER PLACEMENT ANGLE LOGIC
        ...
        # End of MODIFIED FINGER PLACEMENT ANGLE LOGIC

        rot_angle_z_placing = angle_rad + np.pi / 2.0 # Keep surface normal facing center
    ...
    if not mesh_generation_successful:
        print(f" -> r={r:.2f},P={np.round(current_pressures).astype(int)},pos=({chosen_indices[0]},{chosen_indices[1]},{chosen_indices[2]}) -> 网格生成失败, Cost=10.0") # MODIFIED
        return 10.0

    object_point_status = ['Non-Contact'] * num_object_points_global_static
    closest_contact_per_finger_eval = [None] * 3 # Store (dist, obj_pt_idx, finger_idx, cell_id, pt_on_mesh, normal_finger)
    min_dist_per_finger_eval = [float('inf')] * 3
    finger_dot_products = [[] for _ in range(3)] # Store dot products for each finger
    has_overlap_eval = False
    num_contact_points = 0
    dot_prod_tolerance = 1e-6 # Tolerance for dot product check

    if object_points_global_static is None or num_object_points_global_static == 0:
        print(f" -> r={r:.2f},P={np.round(current_pressures).astype(int)},pos=({chosen_indices[0]},{chosen_indices[1]},{chosen_indices[2]}) -> 物体点云为空, Cost=10.0") # MODIFIED
        return 10.0
    ...
    for obj_pt_idx, obj_point in enumerate(object_points_global_static):
        ...
        for finger_idx, finger_mesh in enumerate(deformed_finger_meshes_world_this_eval):
            ...
            try:
                ...
                if closest_cell_id < 0 or closest_cell_id >= finger_mesh.n_cells: continue # Invalid cell
                ...
                if dist < closest_dist_for_this_pt:
                    ...
                    current_normal = None
                    if has_normals and closest_cell_id < len(finger_mesh.cell_data['Normals']): # Check index bounds
                        current_normal = finger_mesh.cell_normals[closest_cell_id]
                    normal_for_this_pt = current_normal
                # Update contact info for GII (closest point on this finger to any object point)
                if dist < collision_threshold and dist < min_dist_per_finger_eval[finger_idx]:
                    ...
                    if current_normal_for_gii is not None: # Only store if normal is valid
            ...
            except Exception: # Catch any error during find_closest_cell or processing
                continue # Skip to next finger or object point

        if finger_idx_for_this_pt != -1: # If this object point found a closest finger
            ...
            if dist < overlap_threshold:
                ...
                if normal_closest is not None and LA.norm(normal_closest) > 1e-9:
                    ...
                    if LA.norm(vector_to_point) > 1e-9: # Avoid zero vector issues
    ...
    cost = 3.0 # Default cost for no contact
    ...
    if has_overlap_eval:
        ...
        for i in range(3):
            if finger_dot_products[i]: # If there are any dot products for this finger
        ...
        if any(finger_intersects):
            ...
            cost = 4.0
        elif num_contact_points > 0 : # If there is contact and no intersection/overlap
            grasp_state_eval = "Contact"
            num_gii_contacts = len(final_contacts_for_gii_eval)
            if num_gii_contacts >= 2: # Need at least 2 contacts for GII
                ...
                if gii is not None and gii > 1e-9: # Valid GII
                    cost = -gii # Negative GII as cost (minimize this means maximize GII)
                else: # GII calculation failed or result too small
                    cost = 2.0
    ...
    eval_params_str = f"r={r:.2f},P={np.array([p1,p2,p3]).round(0).astype(int)},pos=({chosen_indices[0]},{chosen_indices[1]},{chosen_indices[2]})" # MODIFIED
...
def find_initial_grasp(initial_r, initial_finger_indices, pressure_step, max_pressure_init): # MODIFIED SIGNATURE
    print(f"\n--- 开始寻找初始接触点 (r = {initial_r:.2f}, indices = {initial_finger_indices}) ---") # MODIFIED
    global initial_coords_ref_global, model_global, scaler_X_global, scaler_y_global, device_global, \
    ...
    last_closest_contacts = [None] * 3 # To store (dist, obj_pt_idx, finger_idx, cell_id, pt_on_mesh, normal)
    ...
    while True:
        ...
        if init_iter_count > max_iterations:
            ...
            return { 'status': 'IterationLimit', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED

        pressure_changed_init = False
        print(f"  初始点搜索 Iter {init_iter_count}: P=[{current_pressures_init[0]:.0f}, {current_pressures_init[1]:.0f}, {current_pressures_init[2]:.0f}] (r={initial_r:.2f}, indices={initial_finger_indices})") # MODIFIED
        for i in range(3):
        ...
        if not pressure_changed_init and all(finger_contact_achieved_init):
            pass
        elif not pressure_changed_init and not all(finger_contact_achieved_init):
            print("--- 初始点搜索: 压力达到上限或无变化，但未实现三指接触 ---")
            return { 'status': 'MaxPressureReachedOrNoChange', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED

        deformed_finger_meshes_init = [None] * 3
        valid_preds = True
        for i in range(3): # For each finger
        ...
        if not valid_preds:
            ...
            return { 'status': 'PredictionFailed', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED
        ...
        current_closest_contacts = [None] * 3 # (dist, obj_pt_idx, finger_idx, cell_id, pt_on_mesh, normal)
        current_min_dists = [float('inf')] * 3 # Min dist for each finger to any obj point
        ...
        contacting_fingers_indices_this_iter = set() # Which fingers made contact in this iteration
        ...
        if object_points_global_static is not None and num_object_points_global_static > 0:
            for obj_pt_idx, obj_point in enumerate(object_points_global_static):
                    ...
                    try:
                        ...
                        if dist < closest_dist_for_this_pt:
                            ...
                            if has_normals and closest_cell_id < len(finger_mesh.cell_data['Normals']):
                                current_normal = finger_mesh.cell_normals[closest_cell_id]
                            normal_for_this_pt = current_normal
                        # Check if this is the closest contact for this finger for GII info
                ...
                if finger_idx_for_this_pt != -1: # If a closest finger was found for this object point
                    ...
                    if dist_final_for_pt < overlap_threshold:
                        ...
                        # break # from obj_pt_idx loop if overlap is found (optional, faster but less info)
                ...
                if has_overlap_init: break # Break from object point loop if overlap detected

        if has_overlap_init:
            print("  初始点搜索: 检测到重叠。")
            return { 'status': 'Overlap', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': current_point_status, 'contact_info': current_closest_contacts } # MODIFIED

        last_point_status = current_point_status
        last_closest_contacts = current_closest_contacts

        finger_intersects_init = [False] * 3
        if num_contact_points_init > 0: # Only check intersection if there's contact
            for i in range(3):
        ...
        if any(finger_intersects_init):
            print("  初始点搜索: 检测到交叉穿透。")
            return { 'status': 'Intersection', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED

        for i in range(3):
            if i in contacting_fingers_indices_this_iter: # If finger 'i' made contact in this iteration
        ...
        if all(finger_contact_achieved_init):
            print(f"--- 成功找到初始接触点组合 (r={initial_r:.2f}, indices={initial_finger_indices}, P={current_pressures_init.round(0)}) ---") # MODIFIED
            return { 'status': 'FoundInitialPoint', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED

        if not pressure_changed_init and not all(finger_contact_achieved_init):
            print("--- 初始点搜索: 压力达到上限或无变化，但未实现三指接触 (循环终止检查) ---")
            return { 'status': 'MaxPressureReachedOrNoChangeLoopEnd', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED

    return { 'status': 'LoopEndUnknown', 'r': initial_r, 'finger_indices': initial_finger_indices, 'pressures': current_pressures_init.tolist(), 'finger_meshes': last_meshes, 'object_point_status': last_point_status, 'contact_info': last_closest_contacts } # MODIFIED
...
if __name__ == '__main__':
    ...
    if len(object_points_transformed_full_mm) > TARGET_POINT_COUNT_FOR_SIM:
        ...
        if original_hr_colors_float_o3d is not None and indices.size > 0:
            ...
            if sampled_colors_float_o3d.ndim == 2 and sampled_colors_float_o3d.shape[1] == 3:
                 ...
                 sampled_colors_uint8_pv = None; sampled_colors_float_o3d = None # Invalidate if not usable
    ...
    if num_object_points_global_static > 0:
        ...
        pv_obb_object_mesh_global = pv.Cube(center=(0,0,0),
    ...
    bottom_node_idx = np.argmin(initial_coords_ref_global[:,0]) # Furthest point in negative X for reference
    ref_mid_pt = initial_coords_ref_global[bottom_node_idx] + width_translation_vector_global/2.0 # Midpoint of that end segment
    T1_translate_global = create_transformation_matrix_opt8(None,-ref_mid_pt) # Translate this point to origin
    rot_ref_to_local = np.array([[0,1,0],[0,0,1],[1,0,0]]) # Rotate ref X to local Z, ref Y to local X, ref Z to local Y
    ...
    print(f"\n--- 开始在 {NUM_INITIAL_SEARCHES} 个随机 (r, finger_indices) 组合上寻找初始接触点 ---") # MODIFIED
    ...
    if distances_from_new_obj_centroid_xy.size > 0:
        ...
        r_search_max_heuristic = tray_radius * 1.2 # Allow fingers to be outside object projection
    else:
        print("警告: 无法从点云计算有效半径，使用基于托盘半径的默认值。")
        r_search_min_heuristic = R_BOUNDS[0]
        r_search_max_heuristic = R_BOUNDS[1]

    r_search_min = np.clip(r_search_min_heuristic, R_BOUNDS[0], R_BOUNDS[1] * 0.95) # Ensure min is within main bounds
    r_search_max = np.clip(r_search_max_heuristic, r_search_min + (R_BOUNDS[1] - R_BOUNDS[0]) * 0.05, R_BOUNDS[1]) # Ensure max is valid and within main bounds

    if r_search_min >= r_search_max: # Fallback if clipping makes range invalid
    ...
    # angle_offset_search_min = ANGLE_OFFSET_BOUNDS[0] # REMOVED
    # angle_offset_search_max = ANGLE_OFFSET_BOUNDS[1] # REMOVED
    print(f"更新后的初始搜索范围: r in [{r_search_min:.2f}, {r_search_max:.2f}], finger indices from {N_FINGER_SLOTS} slots (r 是相对于新托盘中心的放置半径)") # MODIFIED
    ...
    for k_init in range(NUM_INITIAL_SEARCHES):
        ...
        # angle_val = np.random.uniform(angle_offset_search_min,angle_offset_search_max) # REMOVED
        
        # MODIFIED: Select 3 distinct random indices for finger positions
        possible_indices = list(range(N_FINGER_SLOTS))
        chosen_k_init_indices = random.sample(possible_indices, 3) # random.sample ensures distinctness
        chosen_k_init_indices.sort() # Optional: sort for consistency if needed, though order matters for p1,p2,p3 mapping

        search_res = find_initial_grasp(initial_r=r_val,initial_finger_indices=chosen_k_init_indices,pressure_step=PRESSURE_STEP_INIT_SEARCH,max_pressure_init=max_pressure) # MODIFIED CALL
        last_search_attempt_result = search_res
        if search_res and search_res['status']=='FoundInitialPoint':
            # Parameters for BO: r, p1, p2, p3, idx1, idx2, idx3
            params_entry=[search_res['r']] + search_res['pressures'] + search_res['finger_indices'] # MODIFIED
            collected_initial_points.append(params_entry)
            print(f"*** 找到有效初始点: r={params_entry[0]:.2f},P={np.round(params_entry[1:4],0)},indices=({params_entry[4]},{params_entry[5]},{params_entry[6]}) ***") # MODIFIED
        else:
    ...
    if last_search_attempt_result and 'finger_meshes' in last_search_attempt_result:
        ...
        title_init_pv_viz = f"PyVista - Initial Search (Status: {last_search_attempt_result.get('status','N/A')})" # MODIFIED
        ...
        # MODIFIED TEXT for visualization
        r_disp = last_search_attempt_result.get('r','?')
        indices_disp = last_search_attempt_result.get('finger_indices', ['?']*3)
        pressures_disp = np.array(last_search_attempt_result.get('pressures',['?']*3)).round(0)
        status_disp = last_search_attempt_result.get('status','N/A')
    ...
    if proceed_bo:
        ...
        x0_for_bo = None
        if collected_initial_points: # collected_initial_points elements are [r, p1, p2, p3, idx1, idx2, idx3]
    ...
    if res_bo:
        best_p_list_bo = res_bo.x # This will be [r, p1, p2, p3, idx1, idx2, idx3]
        best_c_bo = res_bo.fun
        best_p_dict_bo = dict(zip(param_names,best_p_list_bo)) # param_names already updated
        print("\n使用最优参数重新评估最终状态...")
        # evaluate_grasp expects named args if called directly, or a list if through gp_minimize or @use_named_args context
        # For direct call, ensure parameters are passed in correct order as defined by evaluate_grasp signature
        final_c_check_bo = evaluate_grasp(best_p_list_bo)
        best_gii_eval_bo = -final_c_check_bo if final_c_check_bo < -1e-9 else 0.
        print("\n找到的最优参数:")
        [print(f"  {n:<12} = {v:.4f}" if not n.startswith("pos_idx") else f"  {n:<12} = {int(v)}") for n,v in best_p_dict_bo.items()] # MODIFIED print format for indices
        ...
        final_chosen_indices_bo_viz = [int(best_p_dict_bo['pos_idx1']), int(best_p_dict_bo['pos_idx2']), int(best_p_dict_bo['pos_idx3'])] # MODIFIED
        final_meshes_bo_viz = []
        all_preds_ok_bo_viz = True

        if len(set(final_chosen_indices_bo_viz)) < 3:
            print("错误：BO最优参数的手指位置索引不唯一，无法生成有效可视化。")
            all_preds_ok_bo_viz = False
        else:
            for i_f_bo_viz in range(3): # For each finger
                ...
                # MODIFIED FINGER PLACEMENT ANGLE LOGIC for BO visualization
                current_pos_index_bo = final_chosen_indices_bo_viz[i_f_bo_viz]
                ang_d_f_bo = current_pos_index_bo * (360.0 / N_FINGER_SLOTS)
                # ang_d_f_bo=([0,120,240][i_f_bo_viz])+best_p_dict_bo['angle_offset'] # REMOVED
                # End of MODIFIED FINGER PLACEMENT ANGLE LOGIC

                ang_r_f_bo=np.radians(ang_d_f_bo)
                rot_z_f_bo=create_rotation_matrix_z(ang_r_f_bo+np.pi/2.) # Surface normal to center
        ...
        if all_preds_ok_bo_viz and len(final_meshes_bo_viz)==3:
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (N-Slots)") # MODIFIED TITLE
            ...
            visualize_poses_with_open3d(
                ...
                tray_radius_mm=tray_radius,
                tray_height_mm=tray_height,
                window_title="Open3D - Optimal SWAPPED & ORIENTED Poses & Axes (N-Slots)" # MODIFIED
            )
        else:
