...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_BO_Torus - 使用程序化甜甜圈进行测试")
...
finger_width = 10.0
TARGET_POINT_COUNT_FOR_SIM = 3500 # 增加点云密度以更好地表现环形
show_axes = True
...
if __name__ == '__main__':
    ...
    world_obb_object_global = o3d_temp_pcd_for_obb.get_axis_aligned_bounding_box() # 对环形使用AABB作为近似
    world_obb_object_global.color = object_obb_color_o3d
    
    # (已修复) 从 Open3D 的 AABB 对象正确获取边界给 PyVista
    ...
    if res_bo:
        ...
        print(f"  pos_indices= {best_pos_indices_bo}")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (Torus)", off_screen=False)
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (Torus)"
            )
        else:
