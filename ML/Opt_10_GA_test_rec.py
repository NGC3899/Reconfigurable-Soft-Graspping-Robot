# -*- coding: utf-8 -*-
# 本脚本使用遗传算法的方式进行构型推荐程序的目标函数求解，使用理想长方体点云测试
import numpy as np
...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_GA_Cuboid - 使用遗传算法和程序化长方体进行测试")
...
CHARACTERISTIC_LENGTH_FOR_GII = 1.0 # 将在主程序中根据物体尺寸计算
...
if __name__ == '__main__':
    ...
    if best_overall_chromosome is not None:
        ...
        print(f"最优成本 (GA): {-best_overall_gii:.4f} (GII: {best_overall_gii:.4f})")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (GA - Cuboid)", off_screen=False)
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (GA - Cuboid)"
            )
        else:
