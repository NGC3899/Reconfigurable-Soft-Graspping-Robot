# -*- coding: utf-8 -*-
# 本脚本使用贝叶斯优化的方式进行构型推荐程序的目标函数求解，使用真实物体点云 + GraspNet预测的位姿
import numpy as np
...
# --- 打印版本信息 ---
try:
    print(f"Opt_10.txt (BO Fix Return Scalar & Verbose Pressure) - Patched - 无重复索引 - 失败步骤可视化 - v4 (位姿修正)")
...
def visualize_failed_bo_step(r_param, chosen_indices_param, pressures_param, finger_meshes_param, failure_reason_str):
    ...
    if object_mesh_global_static:
        if 'colors' in object_mesh_global_static.point_data and object_mesh_global_static.point_data['colors'] is not None:
            plotter_debug.add_mesh(object_mesh_global_static, scalars='colors', rgb=True, style='points', point_size=5, name='obj_debug_color')
        else:
            plotter_debug.add_mesh(object_mesh_global_static, color=object_point_color, style='points', point_size=5, name='obj_debug_default')
...
if __name__ == '__main__':
    ...
    # 物体点云将变换到 T_original_tray_pose_world (即物体出现在托盘的初始位置)
    # 这个 T_object_coord_system_in_world 是物体点云（在其质心和对齐后的局部坐标系中）的目标世界位姿。
    T_object_target_world_pose = T_original_tray_pose_world
    
    # 托盘的参考坐标系 (手指放置基准) 将设置在 T_original_object_pose_world
    ...
    # 用于可视化托盘自身坐标轴的变换矩阵 (参考 Opt_10.txt，使用修正前的托盘参考位姿)
    T_tray_axes_vis_world = _T_tray_ref_before_fix
    # 用于可视化物体自身坐标轴的变换矩阵 (与物体点云的目标世界位姿一致)
    ...
    # 修改：直接使用 T_object_target_world_pose 变换居中且对齐的点云
    # 这会将物体的质心放置在 T_object_target_world_pose 的原点，
    # 并将其（对齐后的）局部X轴与 T_object_target_world_pose 的X轴对齐。
    ...
    if len(object_points_transformed_full_mm) > TARGET_POINT_COUNT_FOR_SIM:
        ...
        # 颜色采样逻辑需要小心，确保索引对应正确
        # 如果 points_high_res_centered_aligned_mm 是从 current_high_res_points_mm_orig_frame 通过一系列变换得到的，
        # 那么直接用 indices 去索引 original_hr_colors_float_o3d 可能不正确，除非变换不改变点的顺序。
        # 一个更稳妥（但可能不完全精确原始颜色）的方法是，如果原始点云有颜色，则在对原始点云采样时也对颜色采样。
        # 这里我们假设，如果颜色存在，其顺序与 current_high_res_points_mm_orig_frame 一致。
        # 并且，points_high_res_centered_aligned_mm 的点顺序与 points_high_res_centered_mm 一致，
        # points_high_res_centered_mm 的点顺序与 current_high_res_points_mm_orig_frame 一致。
        # 因此，indices 可以用于原始颜色数组。
        if original_hr_colors_float_o3d is not None and indices.size > 0 and original_hr_colors_float_o3d.shape[0] == current_high_res_points_mm_orig_frame.shape[0]:
            try:
                sampled_colors_float_o3d = original_hr_colors_float_o3d[indices] # 使用原始点云的颜色索引
    ...
    if res_bo:
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp", off_screen=False)
            plotter_final_pv.add_mesh(tray_pv_global,color=tray_color,opacity=0.5,name='tray_final_pv_oriented')
            if object_mesh_global_static:
                if 'colors' in object_mesh_global_static.point_data and object_mesh_global_static.point_data['colors'] is not None:
                    plotter_final_pv.add_mesh(object_mesh_global_static,scalars='colors',rgb=True,style='points',point_size=5,name='obj_final_pv_c_swapped') 
            ...
            visualize_poses_with_open3d(
                ...
                tray_axes_transform_mm=T_tray_axes_vis_world,    # 使用修正后的托盘坐标轴变换
                bottle_axes_transform_mm=T_object_axes_vis_world, # 使用修正后的物体坐标轴变换
                ...
                window_title="Open3D - Optimal Poses & Axes (Corrected v4)"
            )
        else:
