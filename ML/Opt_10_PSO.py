# -*- coding: utf-8 -*-
# 本脚本使用粒子群优化的方式进行构型推荐程序的目标函数求解，使用真实物体点云 + GraspNet预测的位姿
import numpy as np
...
import pandas as pd # <--- 新增：导入pandas用于Excel导出
...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_PSO_Imported_Object - 粒子群优化版本 (带Excel日志)") # 版本名更改
    print(f"Open3D version: {o3d.__version__}")
    print(f"PyVista version: {pv.__version__}")
    print(f"NumPy version: {np.__version__}")
    print(f"Torch version: {torch.__version__}")
    print(f"Pandas version: {pd.__version__}") # <--- 新增：打印Pandas版本
...
HIGH_RES_OBJECT_FILENAME = "Bird_Model.ply" # 示例，您会从外部读取
...
TARGET_POINT_COUNT_FOR_SIM = 800; OBJECT_SCALE_FACTOR = 0.8
...
PSO_C1 = 1.5
PSO_C2 = 1.5
PSO_V_MAX_R_RATIO = 0.2
PSO_V_MAX_COMBO_RATIO = 0.2
EXCEL_LOG_FILENAME = "pso_optimization_log.xlsx" # <--- 新增：Excel日志文件名
...
def visualize_failed_pso_step(r_param, chosen_indices_param, pressures_param, finger_meshes_param, failure_reason_str, iteration, particle_idx):
    ...
    if tray_pv_mesh_global: plotter_debug.add_mesh(tray_pv_mesh_global, color=tray_color_runtime, opacity=0.5, name='tray_debug')
    ...
    try:
        ...
        if DEBUG_VISUALIZE_FAILED_GRASPS:
            ...
            if isinstance(iteration_info, str) and "-" in iteration_info:
                parts = iteration_info.split("-")
                if len(parts) == 2: iter_num_viz, particle_idx_viz = parts[0].strip(), parts[1].strip()
            visualize_failed_pso_step(r, chosen_indices, current_pressures_fallback,
                                     deformed_meshes_fallback,
                                     failure_reason_for_vis, iter_num_viz, particle_idx_viz)
...
if __name__ == '__main__':
    ...
    # (此部分与之前版本相同，此处省略以减少代码长度，实际代码中保留)
    ...
    T_tray_axes_vis_world = T_actual_tray_geometry_world
    ...
    if OBJECT_SCALE_FACTOR != 1.0: object_points_raw_mm *= OBJECT_SCALE_FACTOR
    ...
    if ALIGN_OBJECT_OBB_TO_X and points_centered_raw_mm.shape[0] > 0:
        local_pcd_for_alignment = o3d.geometry.PointCloud(); local_pcd_for_alignment.points = o3d.utility.Vector3dVector(points_centered_raw_mm)
        try:
    ...
    if sampled_colors_uint8_pv is not None and object_mesh_global_static_pv is not None and len(sampled_colors_uint8_pv) == object_mesh_global_static_pv.n_points:
        object_mesh_global_static_pv.point_data['colors'] = sampled_colors_uint8_pv
    ...
    else: object_centroid_global_static = np.array([0,0,0]); num_object_points_global_static = 0
    if num_object_points_global_static > 0:
        ...
        if world_obb_object_global.extent is not None and all(e > 1e-6 for e in world_obb_object_global.extent): CHARACTERISTIC_LENGTH_FOR_GII = np.mean(world_obb_object_global.extent);
    ...
    if SHOW_INITIAL_SETUP_PREVIEW:
        ...
        if object_points_global_static is not None and object_points_global_static.shape[0] > 0:
            ...
            if tray_pv_mesh_global is None:
                 ...
                 plotter_initial_pv.add_mesh(tray_pv_mesh_global.copy(), color=tray_color_fig1_init, opacity=0.3, name="initial_tray_mesh")
            else:
            ...
            # print("\n(PyVista) 显示初始物体、托盘和示例未变形手指。请关闭窗口以继续。") # 已被Open3D可视化替代或补充
    ...
    optimization_log = [] # <--- 新增：用于存储每代最优结果的列表
    ...
    gbest_pressures = None
    gbest_meshes = None
    gbest_contacts = None

    v_max_r = PSO_V_MAX_R_RATIO * (R_BOUNDS[1] - R_BOUNDS[0])
    v_min_r = -v_max_r
    v_max_combo = PSO_V_MAX_COMBO_RATIO * (num_valid_combinations -1)
    v_min_combo = -v_max_combo
    ...
    for iteration in range(PSO_N_ITERATIONS):
        ...
        iter_gbest_fitness_for_log = -float('inf') # 用于记录当前迭代中实际找到的gbest
        ...
        for i in range(PSO_N_PARTICLES):
            ...
            if current_fitness > particles_pbest_fitness[i]:
                ...
                particles_pbest_position[i, :] = particles_position[i, :].copy()
                particles_pbest_pressures[i] = pressures
                particles_pbest_meshes[i] = meshes
                particles_pbest_contacts[i] = contacts
            ...
            if particles_pbest_fitness[i] > gbest_fitness:
                ...
                gbest_pressures = particles_pbest_pressures[i]
                gbest_meshes = particles_pbest_meshes[i]
                gbest_contacts = particles_pbest_contacts[i]
                print(f"  ** 新的全局最优! Iter {iteration+1}, Particle {i+1}, GII = {gbest_fitness:.4f}, r={gbest_position[0]:.3f}, combo_idx={int(gbest_position[1])} **")
            
            # 更新当次迭代日志用的gbest_fitness
            if gbest_fitness > iter_gbest_fitness_for_log :
                iter_gbest_fitness_for_log = gbest_fitness


        for i in range(PSO_N_PARTICLES):
            r1 = random.random()
            r2 = random.random()
            particles_velocity[i, 0] = (PSO_W * particles_velocity[i, 0] +
                                        PSO_C1 * r1 * (particles_pbest_position[i, 0] - particles_position[i, 0]) +
                                        PSO_C2 * r2 * (gbest_position[0] - particles_position[i, 0]))
            particles_velocity[i, 0] = np.clip(particles_velocity[i, 0], v_min_r, v_max_r)

            particles_velocity[i, 1] = (PSO_W * particles_velocity[i, 1] +
                                        PSO_C1 * r1 * (particles_pbest_position[i, 1] - particles_position[i, 1]) +
                                        PSO_C2 * r2 * (gbest_position[1] - particles_position[i, 1]))
        ...
        # <--- 新增：记录当前迭代的全局最优信息到日志 ---
        current_gbest_r_log = gbest_position[0]
        current_gbest_combo_idx_log = int(round(gbest_position[1]))
        current_gbest_combo_idx_log = max(0, min(current_gbest_combo_idx_log, num_valid_combinations - 1))
        current_gbest_pos_indices_log = all_valid_finger_combinations_global[current_gbest_combo_idx_log]
        
        # gbest_fitness 是适应度 (GII本身，如果是成功抓取的话)
        # 如果gbest_fitness依然是初始的-inf或者是一个负值（代表失败成本），则GII为nan
        gii_for_log = gbest_fitness if gbest_fitness > 1e-9 else np.nan


        log_entry = {
            "Iteration": iteration + 1,
            "Radius (r)": current_gbest_r_log,
            "Finger_Pos_1": current_gbest_pos_indices_log[0],
            "Finger_Pos_2": current_gbest_pos_indices_log[1],
            "Finger_Pos_3": current_gbest_pos_indices_log[2],
            "GII": gii_for_log
        }
        optimization_log.append(log_entry)
        # --- 新增结束 ---

        print(f"  迭代 {iteration + 1} 结束。当前全局最优 GII: {gii_for_log if not np.isnan(gii_for_log) else 'N/A (未找到有效GII)' :.4f}")
    ...
    if optimization_log:
        ...
        try:
            df_log.to_excel(EXCEL_LOG_FILENAME, index=False)
            print(f"\n优化日志已导出至: {EXCEL_LOG_FILENAME}")
    ...
    # --- G. 处理和可视化优化结果 ---
    # (此部分与之前PSO版本相同，此处省略以减少代码长度，实际代码中保留)
    if gbest_fitness > -float('inf'): # 检查是否找到了任何有效的解（即使是失败的，只要不是初始的-inf）
        ...
        final_gii_value = gbest_fitness if gbest_fitness > 1e-9 else np.nan # GII值用于显示

        print("\n找到的最优参数 (PSO):")
        print(f"  r = {best_r_pso:.4f}, pos_indices = {best_pos_indices_pso}, (combo_idx = {best_combo_idx_pso})")
        print(f"最优 GII (PSO): {final_gii_value if not np.isnan(final_gii_value) else 'N/A (非成功GII)' :.4f} (原始适应度: {gbest_fitness:.4f})")
        
        if final_gii_value > 1e-9 : # 仅当找到有效GII时才进行完整可视化
            print("\n使用最优参数生成最终可视化...")
            final_pressures_viz = gbest_pressures
            final_meshes_viz = gbest_meshes
            ...
            if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
                ...
                gii_disp_pso=f"GII={final_gii_value:.3f}"

                plotter_final_pv = setup_pv_plotter(title="PyVista - Optimal Grasp (PSO - Imported Object)")
                ...
                if object_mesh_global_static_pv:
                    if 'colors' in object_mesh_global_static_pv.point_data and object_mesh_global_static_pv.point_data['colors'] is not None:
                        plotter_final_pv.add_mesh(object_mesh_global_static_pv, scalars='colors', rgba=True, style='points', point_size=5, name='obj_final_pv_imported')
                    else:
                        plotter_final_pv.add_mesh(object_mesh_global_static_pv, color=object_point_color_runtime, style='points', point_size=5, name='obj_final_pv_imported')
                ...
                plotter_final_pv.add_text(f"Optimal (PSO):{params_txt_pso}\n{gii_disp_pso}",position="upper_left",font_size=10,color=text_color_fig1_init)
                ...
                visualize_poses_with_open3d(
                    ...
                    object_obb_world=world_obb_object_global,
                    tray_radius_mm=tray_radius, tray_height_mm=tray_height,
                    window_title="Open3D - Optimal Poses & Axes (PSO - Corrected Opt10BO Logic)")
            else:
                print("未能为最优参数生成手指网格或获取最终压力，无法进行PyVista最终可视化。")
        else:
             print("最终找到的最优解不是一个成功的GII抓取，不进行最终的成功抓取可视化。")

        if gbest_fitness is not None : print(f"  (找到的最终最优适应度为: {gbest_fitness:.3f})")
