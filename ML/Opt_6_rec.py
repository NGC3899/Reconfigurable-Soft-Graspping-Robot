...
# === START: 修改物体参数 ===
OBJECT_TYPE = 'cuboid' # 'sphere' or 'cuboid'
# 球体参数 (如果 OBJECT_TYPE = 'sphere')
...
# 长方体参数 (如果 OBJECT_TYPE = 'cuboid')
cuboid_dims = np.array([35.0, 30.0, 40.0]) # Length(X), Width(Y), Height(Z)
num_cuboid_points = 500 # 长方体表面点数
cuboid_rotation_angle_z = np.radians(30) # 长方体绕Z轴旋转角度 (设为0表示不旋转)
cuboid_center_translation = np.array([0, 0, cuboid_dims[2]/2.0 + 35.0]) # 长方体中心位置
# === END: 修改物体参数 ===

show_axes = True; finger_color = 'lightcoral'; tray_color = 'tan'; object_point_color = 'blue' # 使用通用名称
...
R_BOUNDS = (tray_radius * 0.3, tray_radius * 0.95) # BO 优化时的 r 范围 (可能需要根据物体调整)
...
# --- 5. 辅助函数 ---
# ... (create_rotation_matrix 等几何变换函数不变) ...
...
def generate_cuboid_points(dimensions, num_points):
    """在长方体表面生成点云"""
    ...
    area_yz = w * h
    area_xz = l * h
    total_area = 2 * (area_xy + area_yz + area_xz)

    if total_area <= 1e-6: return np.zeros((0, 3)) # Avoid division by zero

    n_xy = int(np.round(num_points * (area_xy / total_area))) # 上下面点数
    n_yz = int(np.round(num_points * (area_yz / total_area))) # 左右面点数
    n_xz = int(np.round(num_points * (area_xz / total_area))) # 前后面点数

    # 确保总点数接近 num_points (可能因取整有微小差异)
    n_total_approx = 2 * (n_xy + n_yz + n_xz)
    # 可以选择重新分配差异，或接受微小差异，这里简单接受

    points = []

    # 上下面 (Z = +/- hh)
    if n_xy > 0:
        x_top = np.random.uniform(-hl, hl, n_xy)
        y_top = np.random.uniform(-hw, hw, n_xy)
        points.append(np.column_stack((x_top, y_top, np.full(n_xy, hh)))) # Top
        x_bot = np.random.uniform(-hl, hl, n_xy)
        y_bot = np.random.uniform(-hw, hw, n_xy)
        points.append(np.column_stack((x_bot, y_bot, np.full(n_xy, -hh)))) # Bottom

    # 左右面 (X = +/- hl)
    if n_yz > 0:
        y_right = np.random.uniform(-hw, hw, n_yz)
        z_right = np.random.uniform(-hh, hh, n_yz)
        points.append(np.column_stack((np.full(n_yz, hl), y_right, z_right))) # Right
        y_left = np.random.uniform(-hw, hw, n_yz)
        z_left = np.random.uniform(-hh, hh, n_yz)
        points.append(np.column_stack((np.full(n_yz, -hl), y_left, z_left))) # Left

    # 前后面 (Y = +/- hw)
    if n_xz > 0:
        x_front = np.random.uniform(-hl, hl, n_xz)
        z_front = np.random.uniform(-hh, hh, n_xz)
        points.append(np.column_stack((x_front, np.full(n_xz, hw), z_front))) # Front
        x_back = np.random.uniform(-hl, hl, n_xz)
        z_back = np.random.uniform(-hh, hh, n_xz)
        points.append(np.column_stack((x_back, np.full(n_xz, -hw), z_back))) # Back
...
# === START: 添加全局变量存储物体信息 ===
object_mesh_global_static = None # 用于可视化物体轮廓
object_center_global_static = np.array([0.,0.,0.]) # 物体中心 (变换后)
# === END: 添加全局变量存储物体信息 ===
...
def evaluate_grasp(r, p1, p2, p3):
    ...
    if has_overlap_eval: grasp_state_eval = "Overlap"; cost = 5.0
        ...
        if any(finger_intersects): grasp_state_eval = "Intersection"; cost = 4.0
            ...
            if num_gii_contacts >= 3: # GII计算现在至少需要3个接触点，不一定正好是3个
                ...
                else: cost = 2.0 # GII 计算失败或过低
            else: cost = 1.0 + (3.0 - num_gii_contacts) # 接触点少于3个
...
if __name__ == '__main__':
    ...
    if OBJECT_TYPE == 'cuboid':
        print(f"生成长方体点云: 尺寸={cuboid_dims}, 点数={num_cuboid_points}")
        object_points_local = generate_cuboid_points(cuboid_dims, num_cuboid_points)
        object_rotation = create_rotation_matrix_z(cuboid_rotation_angle_z)
        object_translation = cuboid_center_translation
        object_center_local = np.array([0., 0., 0.]) # 中心在局部坐标系原点
    elif OBJECT_TYPE == 'sphere':
        print(f"生成球体点云: 半径={sphere_radius}, 点数={num_sphere_points}")
        object_points_local = generate_sphere_points(sphere_radius, num_sphere_points)
        object_rotation = create_rotation_matrix_z(sphere_rotation_angle_z)
        object_translation = sphere_translation
        object_center_local = np.array([0., 0., 0.])
    else:
        print(f"错误: 未知的 OBJECT_TYPE '{OBJECT_TYPE}'"); sys.exit()

    object_transform = create_transformation_matrix(object_rotation, object_translation)
    ...
    if object_points_global_static is None or object_points_global_static.shape[0] == 0:
        print("错误：无法生成或变换物体点云。"); sys.exit()

    # 计算全局质心和中心
    object_centroid_global_static = np.mean(object_points_global_static, axis=0)
    # 物体中心点变换到全局坐标
    object_center_global_static = transform_points(object_center_local.reshape(1, 3), object_transform)[0]

    # 创建用于可视化的物体网格轮廓
    if OBJECT_TYPE == 'cuboid':
        object_mesh_global_static = pv.Cube(center=object_center_global_static,
                                            x_length=cuboid_dims[0],
                                            y_length=cuboid_dims[1],
                                            z_length=cuboid_dims[2]).rotate_z(np.degrees(cuboid_rotation_angle_z), inplace=False) # 应用旋转
    elif OBJECT_TYPE == 'sphere':
        object_mesh_global_static = pv.Sphere(center=object_center_global_static, radius=sphere_radius)
    # === END: 生成并放置选择的物体 ===


    num_object_points_global_static = object_points_global_static.shape[0]
    print(f"已生成 {OBJECT_TYPE} 物体，包含 {num_object_points_global_static} 个点。")
    print(f"全局质心: {object_centroid_global_static.round(3)}")
    print(f"全局中心: {object_center_global_static.round(3)}")


    # Check essential global variables are initialized
    ...
    # --- 步骤 1: 寻找初始接触点 (多次随机半径尝试) ---
    print(f"\n--- 开始在 {NUM_RANDOM_R_SEARCHES} 个随机半径上寻找初始接触点 ---")
    collected_initial_points = []
    last_search_attempt_result = None

    # === START: 修改 r_search_min 计算 ===
    if OBJECT_TYPE == 'cuboid':
        # 长方体在XY平面的最大半径
        cuboid_max_xy_radius = 0.5 * np.sqrt(cuboid_dims[0]**2 + cuboid_dims[1]**2)
        r_search_min = cuboid_max_xy_radius * 1.1 # 比物体XY最大半径稍大
    elif OBJECT_TYPE == 'sphere':
        r_search_min = sphere_radius * 1.1 # 比球半径稍大
    else:
        r_search_min = R_BOUNDS[0] # 使用默认下限

    r_search_max = tray_radius * 0.95 # 比托盘半径稍小
    # === END: 修改 r_search_min 计算 ===
    ...
    for r_init in random_radii:
        ...
        if search_result and search_result['status'] == 'FoundInitialPoint':
            ...
            else: print(f"警告: 找到的初始点参数 ({r_found:.2f}, {p_found}) 不在贝叶斯优化边界内，将不用于 x0。")
    ...
    if last_search_attempt_result and 'finger_meshes' in last_search_attempt_result:
        ...
        if object_mesh_global_static:
            plotter_init_viz.add_mesh(object_mesh_global_static, style='wireframe', color='cyan', line_width=2, name='object_outline_init')
        # === 结束添加 ===
        ...
        if init_rgba_colors.shape == (num_object_points_global_static, 4):
             ...
             plotter_init_viz.add_mesh(init_point_cloud_polydata, scalars='colors', rgba=True, style='points', point_size=5, render_points_as_spheres=True, name='object_points') # 缩小点尺寸
    ...
    if result is not None:
        ...
        final_cost_check = evaluate_grasp([final_r, final_p1, final_p2, final_p3]) # 正确调用方式
        ...
        if all_final_preds_valid and len(final_finger_meshes) == 3:
            ...
            if final_state_str == "Contact":
                 # GII 计算逻辑调整：只要接触点 >= 3 就可以尝试计算
                 if len(final_contacts_for_gii) >= 3:
                      final_gii = calculate_gii_multi_contact(final_contacts_for_gii, object_centroid_global_static, friction_coefficient, eigenvalue_threshold)
            # (GII 文本显示逻辑调整)
            ...
            # === 添加物体轮廓可视化 ===
            if object_mesh_global_static:
                plotter_final.add_mesh(object_mesh_global_static, style='wireframe', color='cyan', line_width=2, name='object_outline_final')
            # === 结束添加 ===
            if show_axes: plotter_final.add_axes_at_origin(labels_off=False)
            for i, mesh in enumerate(final_finger_meshes): # 添加手指和法向量
                if mesh is not None:
            ...
            # 添加接触点信息
            contact_marker_colors = ['red', 'green', 'cyan', 'purple', 'orange', 'pink'] # 增加颜色以防接触点多于3个
