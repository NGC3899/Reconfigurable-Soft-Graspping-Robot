...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_Random_Search_Imported_Object - 随机搜索版本 (保留外部依赖)")
...
def visualize_failed_step(r_param, chosen_indices_param, pressures_param, finger_meshes_param, failure_reason_str, iter_info=""):
    ...
    if not DEBUG_VISUALIZE_FAILED_GRASPS or FAILED_GRASP_VIS_COUNT >= MAX_FAILED_GRASP_VIS:
        ...
        return
    FAILED_GRASP_VIS_COUNT += 1; plotter_title = f"随机搜索失败调试 {iter_info} #{FAILED_GRASP_VIS_COUNT}: {failure_reason_str}"
    ...
    plotter_debug.camera_position = 'xy'; print(f"\n(PyVista) 显示失败步骤 {iter_info}。原因: {failure_reason_str}。请关闭窗口以继续...")
    plotter_debug.show(cpos='xy'); plotter_debug.close()

def evaluate_grasp(r, pos_idx1, pos_idx2, pos_idx3, iter_info="N/A"):
    ...
    if T_pose_for_tray_display_and_finger_placement_global is None:
        print(f"错误: T_pose_for_tray_display_and_finger_placement_global 未在 evaluate_grasp 中初始化! (Info: {iter_info})")
    ...
    try:
        ...
        while True:
            ...
            if current_pressure_iter > max_pressure_iterations:
                ...
                print(f"  评估 ({iter_info}): {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print}")
                if DEBUG_VISUALIZE_FAILED_GRASPS: visualize_failed_step(r, chosen_indices, current_pressures, deformed_finger_meshes_at_contact, failure_reason_for_vis, iter_info)
            ...
            if object_points_global_static is not None and num_object_points_global_static > 0:
                ...
                if has_overlap_this_iter:
                    ...
                    print(f"  评估 ({iter_info}): {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print}")
                    if DEBUG_VISUALIZE_FAILED_GRASPS: visualize_failed_step(r, chosen_indices, current_pressures, current_step_finger_meshes, failure_reason_for_vis, iter_info)
            ...
            if all_fingers_established_this_iter_round:
                ...
                if len(final_gii_contacts_collected) >= 2:
                    ...
                    if gii is not None and gii > 1e-9:
                        ...
                        print(f"  评估 ({iter_info}): {current_call_params_str} -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print:.4f}")
                        ...
                        print(f"  评估 ({iter_info}): {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print if isinstance(gii_value_to_print, str) else gii_value_to_print:.4f}")
                        if DEBUG_VISUALIZE_FAILED_GRASPS: visualize_failed_step(r, chosen_indices, return_pressures, return_meshes, failure_reason_for_vis, iter_info)
                        return (cost_to_return, return_pressures, return_meshes, return_contacts)
                else:
        ...
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(f"DEBUG: EXCEPTION in evaluate_grasp for {current_call_params_str} (Info: {iter_info}): {e_eval_grasp}")
        ...
        print(f"  评估 ({iter_info}): {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print}")
        if DEBUG_VISUALIZE_FAILED_GRASPS: visualize_failed_step(r, chosen_indices, current_pressures_fallback, deformed_meshes_fallback, failure_reason_for_vis, iter_info)
...
if __name__ == '__main__':
    ...
    if SHOW_INITIAL_SETUP_PREVIEW:
        ...
        if object_points_global_static is not None and object_points_global_static.shape[0] > 0:
            ...
            visualize_poses_with_open3d(
                ...
                object_colors_rgb_float=sampled_object_colors_float_o3d_global, object_obb_world=world_obb_object_global,
                tray_radius_mm=tray_radius, tray_height_mm=tray_height,
                window_title="Open3D - 初始物体和托盘设置")
    ...
    for i in range(N_CALLS_RANDOM):
        iter_debug_info = f"迭代 {i+1}/{N_CALLS_RANDOM}"
        print(f"\n--- {iter_debug_info} ---")
        ...
        pos_idx1, pos_idx2, pos_idx3 = chosen_indices_rand
        ...
        if cost < best_overall_cost:
            ...
            print(f"  ** 新的最优解! {iter_debug_info}, 成本: {best_overall_cost:.4f}, r={rand_r:.3f}, pos={chosen_indices_rand} **")
    ...
    if best_overall_params is not None:
        best_r_final = best_overall_params['r']
        best_combo_idx_final = best_overall_params['combo_idx']
        best_pos_indices_final = best_overall_params['indices']

        best_gii_final = -best_overall_cost if best_overall_cost < -1e-9 else 0.

        print("\n找到的最优参数 (随机搜索):")
        print(f"  r          = {best_r_final:.4f}")
        print(f"  pos_indices= {best_pos_indices_final}")
        print(f"  (combo_idx = {best_combo_idx_final})")
        print(f"最优成本: {best_overall_cost:.4f}")
        if best_gii_final > 0: print(f"对应 GII: {best_gii_final:.4f}")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            state_disp=f"Cost={best_overall_cost:.3f}"; gii_disp=f"GII={best_gii_final:.3f}" if best_gii_final > 1e-9 else "GII:N/A"
            plotter_final_pv = setup_pv_plotter(title="PyVista - Optimal Grasp (Random Search - Imported Object)")
            ...
            plotter_final_pv.add_text(f"Optimal (Random):{params_txt}\n{state_disp}\n{gii_disp}",position="upper_left",font_size=10,color=text_color_fig1_init)
            ...
            visualize_poses_with_open3d(
                ...
                tray_radius_mm=tray_radius, tray_height_mm=tray_height,
                window_title="Open3D - Optimal Poses & Axes (Random Search - Imported Object)")
        else:
            print("未能为最优参数生成手指网格或获取最终压力，无法进行PyVista最终可视化。")
            if best_overall_cost is not None : print(f"  (找到的最优成本为: {best_overall_cost:.3f})")
