...
def calculate_gii_multi_contact(contacts_info, object_centroid, mu, eigenvalue_threshold):
    ...
    for contact in contacts_info:
        ...
        n_contact = - (normal_finger / norm_mag) # 物体法线 = - 手指法线
    ...
    try:
        ...
        if len(non_zero_eigenvalues) > 0:
            ...
            # 添加检查 lambda_max 是否有效
            if lambda_max <= 0: print(f"警告: lambda_max <= 0 ({lambda_max:.2e})"); return 0.0
...
def evaluate_grasp(r, p1, p2, p3):
    ...
    contact_side_signs = [] # <<< 存储所有接触点的 side_sign
    has_overlap_eval = False # <<< 标记是否发生重叠
            ...
            try:
                ...
                if dist < closest_dist_for_this_pt:
                    ...
                    if closest_cell_id < len(finger_mesh.cell_data['Normals']):
                        ...
                        vector_to_point = obj_point - pt_on_mesh
                        side_sign = np.sign(np.dot(vector_to_point, normal)) if LA.norm(vector_to_point) > 1e-9 else 0
                        info_for_this_pt = (dist, finger_idx, closest_cell_id, pt_on_mesh, normal, side_sign) # <<< 包含 side_sign
                    else: info_for_this_pt = None

                # 更新该手指的最近接触信息 (仅当距离小于阈值)
                if dist < collision_threshold and dist < min_dist_per_finger_eval[finger_idx]:
                     ...
                     if closest_cell_id < len(finger_mesh.cell_data['Normals']):
                        ...
                        closest_contact_per_finger_eval[finger_idx] = (dist, obj_pt_idx, finger_idx, closest_cell_id, pt_on_mesh, normal_f)
                     else: closest_contact_per_finger_eval[finger_idx] = None

            except Exception: continue # 忽略内部错误

        # 根据全局最近信息判断点状态
        if info_for_this_pt:
            dist, finger_idx, cell_id, pt_on_mesh, normal, side_sign = info_for_this_pt # 解包 side_sign
            if dist < overlap_threshold:
                object_point_status[obj_pt_idx] = 'Overlap'
                has_overlap_eval = True # 标记发生重叠
            elif dist < collision_threshold:
                object_point_status[obj_pt_idx] = 'Contact'
                contact_side_signs.append(side_sign) # <<< 记录接触点的 side_sign
            # else: # 大于 collision_threshold
            #    object_point_status[obj_pt_idx] = 'Non-Contact'

    # --- 3. 判断抓取状态并计算 GII 或惩罚值 ---
    final_contacts_for_gii_eval = [info for info in closest_contact_per_finger_eval if info is not None]
    grasp_state_eval = "No Contact" # 默认状态
    ...
    if has_overlap_eval:
        ...
        # 检查是否有接触点
        if contact_side_signs: # 如果列表不为空，说明有接触点
            found_positive = any(s > 0 for s in contact_side_signs)
            found_negative = any(s < 0 for s in contact_side_signs)
            if found_positive and found_negative:
                grasp_state_eval = "Intersection"
                print("交叉穿透！惩罚=4.0")
                return 4.0 # 交叉给予高惩罚
            elif found_positive or found_negative: # 接触点只在一侧 (或在表面side_sign=0)
                 grasp_state_eval = "Contact"
                 # 检查是否有足够的接触点用于 GII 计算
                 if len(final_contacts_for_gii_eval) == 3:
                     print("3有效接触点，计算GII...", end="")
                     ...
                     if gii is not None and gii > 1e-9:
                         ...
                         return 2.0 # GII 计算失败或为零
                 else: # 接触点不足3个
                     penalty = 3.0 - len(final_contacts_for_gii_eval)
                     print(f"{len(final_contacts_for_gii_eval)}接触点 (不足3). 惩罚={penalty + 1.0:.1f}")
                     return penalty + 1.0
            # else: # 接触点都在表面？ 视为 No Contact 或低惩罚？ 暂按 No Contact 处理
        # else: # contact_side_signs 为空，没有点小于 collision_threshold
    # 如果既不重叠，也不交叉，也不是有效接触，则认为是无接触
    if grasp_state_eval == "No Contact":
        print(f"无有效接触或仅表面接触. 惩罚=4.0")
        return 4.0 # 无接触给予高惩罚
...
if __name__ == '__main__':
    ...
    final_contact_side_signs = [] # 用于最终可视化判断
    final_has_overlap = False
    ...
    if all_final_preds_valid and len(final_finger_meshes) == 3:
        # 最终碰撞检测 (与 evaluate_grasp 中的逻辑类似，但需要存储 side_sign)
        for obj_pt_idx, obj_point in enumerate(object_points_global_static):
             ...
             for finger_idx, finger_mesh in enumerate(final_finger_meshes):
                 ...
                 try:
                     ...
                     if dist < collision_threshold and dist < final_min_dist_per_finger[finger_idx]:
                          ...
                          if closest_cell_id < len(finger_mesh.cell_data['Normals']):
                              normal_f = finger_mesh.cell_normals[closest_cell_id]
                              final_closest_contact_per_finger[finger_idx] = (dist, obj_pt_idx, finger_idx, closest_cell_id, pt_on_mesh, normal_f)
                          else: final_closest_contact_per_finger[finger_idx] = None
                 except Exception: continue
             # 判断最终状态
             if info_for_this_pt:
                 dist, _, _, _, _, side_sign = info_for_this_pt # 获取 side_sign
                 if dist < overlap_threshold:
                      final_object_point_status[obj_pt_idx] = 'Overlap'; final_has_overlap = True
                 elif dist < collision_threshold:
                      final_object_point_status[obj_pt_idx] = 'Contact'
                      final_contact_side_signs.append(side_sign) # 记录 side_sign

        # 确定最终状态字符串
        final_contacts_for_gii = [info for info in final_closest_contact_per_finger if info is not None]
        final_gii = best_gii # 使用优化结果
        final_state_str = "Unknown"
        if final_has_overlap: final_state_str = "Overlap"
        elif final_contact_side_signs: # 有接触点
            pos = any(s > 0 for s in final_contact_side_signs)
            neg = any(s < 0 for s in final_contact_side_signs)
            if pos and neg: final_state_str = "Intersection"
            else: final_state_str = "Contact"
        else: final_state_str = "No Contact"
        ...
        # 根据最终状态着色点云
        color_map_final = { 'Non-Contact': list(pv.Color(sphere_color).int_rgb) + [255], 'Contact': list(pv.Color(colliding_point_color).int_rgb) + [255], 'Overlap': list(pv.Color(overlap_point_color).int_rgb) + [255], 'Intersection': list(pv.Color(intersection_point_color).int_rgb) + [255], 'Gray': list(pv.Color('grey').int_rgb) + [255] }
        final_rgba_colors = np.array([color_map_final.get(status, color_map_final['Gray']) for status in final_object_point_status], dtype=np.uint8)
        ...
        if final_state_str == "Contact" and len(final_contacts_for_gii) > 0: # 改为检查最终状态
