# -*- coding: utf-8 -*-
# 本脚本使用贝叶斯优化的方式进行构型推荐程序的目标函数求解，使用理想球形点云测试
import numpy as np
...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_BO_Sphere - 使用程序化球体进行测试 (v2 - Figure_1 位置对齐)")
...
PRESSURE_STEP_EVAL_GRASP = 300.0
...
SPHERE_OBJECT_RADIUS = 35.0 
# 参照 Figure_1.txt 中的 SPHERE_OBJECT_CENTER_OFFSET_Z 定义
SPHERE_OBJECT_CENTER_OFFSET_Z = SPHERE_OBJECT_RADIUS + 25.0 
...
if __name__ == '__main__':
    ...
    # 如果托盘有特定的世界变换 T_actual_tray_geometry_world, 则应用它
    # tray_pv_global = tray_pv_global.transform(T_actual_tray_geometry_world, inplace=False) # 在此场景中，托盘在原点，所以不需要额外变换
    ...
    if res_bo:
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (Sphere)", off_screen=False)
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (Sphere)"
            )
        else:
