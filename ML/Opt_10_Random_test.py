...
# --- 打印版本信息 ---
try:
    print(f"Opt_10_Random_Search_Cuboid - 使用程序化长方体进行测试 (随机搜索版本)")
...
if __name__ == '__main__':
    ...
    # 计算长方体中心的世界坐标 (中心在 x,y=0, z=设定的距离+半高)
    ...
    # --- 更新可视化模型 ---
    object_mesh_global_static = pv.Cube(center=CUBOID_CENTER_WORLD,
                                        x_length=cuboid_l,
                                        y_length=cuboid_w,
                                        z_length=cuboid_h)
    ...
    for i in range(N_CALLS_BO):
        print(f"\n--- 随机搜索迭代: {i+1}/{N_CALLS_BO} ---")
    ...
    if best_params:
        ...
        final_cost_viz, final_pressures_viz, final_meshes_viz, final_contacts_viz = evaluate_grasp(
            r=best_r_final,
            pos_idx1=best_pos_indices_final[0],
            pos_idx2=best_pos_indices_final[1],
            pos_idx3=best_pos_indices_final[2]
        )
        DEBUG_VISUALIZE_FAILED_GRASPS = original_debug_vis_state
        
        best_gii_eval = -final_cost_viz if final_cost_viz < -1e-9 else 0.
        
        print("\n找到的最优参数:")
        print(f"  r          = {best_r_final:.4f}")
        print(f"  pos_idx1   = {best_pos_indices_final[0]}")
        print(f"  pos_idx2   = {best_pos_indices_final[1]}")
        print(f"  pos_idx3   = {best_pos_indices_final[2]}")
        print(f"  (combo_idx = {best_combo_idx_final})")
        print(f"优化找到的最低成本: {best_c_final:.4f}")
        print(f"重评成本 (from full eval): {final_cost_viz:.4f}")
        if best_gii_eval > 0: print(f"重评GII (derived from cost): {best_gii_eval:.4f}")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            state_disp=f"Cost={final_cost_viz:.3f}"; gii_disp=f"GII={best_gii_eval:.3f}" if best_gii_eval > 0 else "GII:N/A"
            plotter_final_pv = pv.Plotter(window_size=[1000,800],title="PyVista - Optimal Grasp (Cuboid, Random Search)", off_screen=False)
            ...
            params_txt=f"r={best_r_final:.2f}, P={pressures_display_str}, indices=({best_pos_indices_final[0]},{best_pos_indices_final[1]},{best_pos_indices_final[2]})"
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (Cuboid, Random Search)"
            )
        else:
