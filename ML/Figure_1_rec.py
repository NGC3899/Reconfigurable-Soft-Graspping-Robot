...
# --- 打印版本信息 ---
try:
    print(f"Case_Study_GII_Analysis_Script (Diagnosing GII=0 Issue)")
...
# --- 4. Case Study 配置参数 ---
CASE_STUDY_OUTPUT_DIR = "grasp_case_study_cuboid_diag" # Changed output dir
EXCEL_FILENAME = "gii_analysis_cuboid_diag.xlsx"
DEBUG_PLOT_SUBDIR = "debug_visualization_plots_cuboid_diag"
tray_radius = 60.0; tray_height = 1.0; finger_width = 10.0
CUBOID_SIZE = [100.0, 15.0, 20.0]
CUBOID_CENTER_OFFSET_Z = (CUBOID_SIZE[2] / 2.0) + 35.0
CUBOID_N_POINTS = 2000
CUBOID_INITIAL_ROTATION_EULER_ANGLES_DEG = [0, 0, -52.5]
CHARACTERISTIC_LENGTH_FOR_GII = max(CUBOID_SIZE) / 2.0
NUM_FINGER_SLOTS_CASE_STUDY = 10
# min_r_val = (max(CUBOID_SIZE[:2])/2.0) + finger_width * 0.5
min_r_val = 51
# max_r_val = tray_radius * 0.85
max_r_val = tray_radius
R_VALUES_TO_TEST = np.linspace(min_r_val, max_r_val, 4) # Reduced for faster diagnosis
INITIAL_PRESSURE_FOR_CONTACT_SEARCH = 100.0
PRESSURE_STEP_FOR_CONTACT_SEARCH = 300.0
...
SHOW_ONE_TIME_INITIAL_CONFIG_PREVIEW = True # Disabled for faster diagnosis
...
def predict_displacements_for_pressure(model,scaler_X,scaler_y,device,pressure_value):
    ...
    return None
def create_cuboid_point_cloud(size, center, rotation_euler_angles_deg, n_points=500):
    cuboid_mesh = o3d.geometry.TriangleMesh.create_box(width=size[0], height=size[1], depth=size[2], create_uv_map=False, map_texture_to_each_face=False)
    cuboid_mesh.translate(-np.array(size) / 2.0, relative=True)
    R_z = cuboid_mesh.get_rotation_matrix_from_axis_angle(np.array([0,0,1]) * np.radians(rotation_euler_angles_deg[2]))
    cuboid_mesh.rotate(R_z, center=(0,0,0))
    R_y = cuboid_mesh.get_rotation_matrix_from_axis_angle(np.array([0,1,0]) * np.radians(rotation_euler_angles_deg[1]))
    cuboid_mesh.rotate(R_y, center=(0,0,0))
    R_x = cuboid_mesh.get_rotation_matrix_from_axis_angle(np.array([1,0,0]) * np.radians(rotation_euler_angles_deg[0]))
    cuboid_mesh.rotate(R_x, center=(0,0,0))
    cuboid_mesh.translate(np.array(center), relative=False)
    pcd = cuboid_mesh.sample_points_uniformly(number_of_points=n_points)
    return np.asarray(pcd.points), cuboid_mesh
...
def calculate_gii_multi_contact(contacts_info, object_centroid_for_gii, mu, eigenvalue_thresh,
    ...
    try:
        ...
        # DEBUG PRINT:
        # print(f"DEBUG GII Calc: Num positive eigenvalues = {len(positive_eigenvalues)}, All eigenvalues = {eigenvalues}")
...
object_mesh_for_visualization = None
...
def evaluate_gii_for_case_study_with_pressure_iteration(r_value, finger_indices_tuple, normal_sign_multipliers=(1,1,1)):
    ...
    # DIAGNOSTIC PRINT
    # print(f"DIAG: Evaluating r={r_value:.2f}, combo={finger_indices_tuple}, signs={normal_sign_multipliers}")
    ...
    if not all(v is not None for v in [initial_coords_ref_global, model_global, object_points_global_static, faces_np_global, width_translation_vector_global, T1_translate_global, T2_rotate_global, T_pose_for_tray_display_and_finger_placement_global]):
        print("DIAG: Exiting due to uninitialized globals.")
        return default_return
    if num_object_points_global_static == 0 :
        print("DIAG: Exiting due to no object points.")
        return default_return
    if len(set(finger_indices_tuple)) < 3:
        print("DIAG: Exiting due to non-unique finger indices.")
        return default_return
    ...
    while True:
        ...
        if current_total_iterations > max_total_iterations:
            print(f"DIAG: Exiting due to max_total_iterations ({max_total_iterations}) reached.")
            return default_return
        ...
        for i in range(3):
            if not finger_contact_established[i]:
                ...
                elif current_pressures[i] >= MAX_PRESSURE_FOR_CONTACT_SEARCH:
                    print(f"DIAG: Finger {i} reached MAX_PRESSURE without contact. Exiting.")
                    return default_return
        ...
        for i in range(3):
            ...
            if displacements is None: all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: predict_displacements_for_pressure returned None for finger {i}."); break
            ...
            if s_d_c1 is None or s_d_c2 is None: all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: sort_points_spatially returned None for finger {i}."); break
            vertices = np.vstack((s_d_c1, s_d_c2))
            if faces_np_global is None or faces_np_global.size == 0 : all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: faces_np_global is None or empty for finger {i}."); break
            try: mesh_ref = pv.PolyData(vertices, faces=faces_np_global)
            except Exception as e_pv: all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: pv.PolyData creation error for finger {i}: {e_pv}"); break
            ...
            try:
                ...
                if mesh_world is None or mesh_world.n_points == 0: all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: transform returned None or empty mesh for finger {i}."); break
            ...
            except Exception as e_tf: all_meshes_generated_ok = False; print(f"DIAG: Mesh gen fail: transform/clean/normals error for finger {i}: {e_tf}"); break
        if not all_meshes_generated_ok:
            print("DIAG: Exiting due to mesh generation failure in loop.")
            return default_return
        ...
        if object_points_global_static is not None:
            for obj_pt_idx, obj_point in enumerate(object_points_global_static):
                ...
                if winning_finger_for_this_obj_pt != -1:
                    if closest_dist_this_obj_pt_to_any_finger < overlap_threshold: has_overall_overlap_this_iteration = True; print(f"DIAG: Overlap detected. ObjPt {obj_pt_idx}, Finger {winning_finger_for_this_obj_pt}, Dist {closest_dist_this_obj_pt_to_any_finger:.4f}"); break
            ...
            if has_overall_overlap_this_iteration:
                print("DIAG: Exiting due to overall overlap.")
                return default_return
        ...
        if not has_overall_overlap_this_iteration:
            for i in range(3):
                if contact_made_by_finger_this_iteration[i] and finger_dot_products_this_iteration[i]:
                    ...
                    if pos_dp and neg_dp: finger_intersects_this_iteration[i] = True; print(f"DIAG: Intersection detected for finger {i}.")
        if any(finger_intersects_this_iteration):
            print("DIAG: Exiting due to intersection.")
            return default_return
        for i in range(3):
        ...
        if not pressure_increased_this_step and not all(finger_contact_established):
            print("DIAG: Exiting: No pressure increase possible, but not all fingers contacted.")
            return default_return
    if not all(finger_contact_established):
        print("DIAG: Exiting: Loop finished, but not all fingers established contact.")
        return default_return
    ...
    if object_points_global_static is not None:
        for finger_idx_gii, finger_mesh_gii in enumerate(deformed_finger_meshes_at_contact):
            if finger_mesh_gii is None or finger_mesh_gii.n_cells == 0:
                print(f"DIAG GII Collect: Finger {finger_idx_gii} mesh is None or has no cells.")
                continue
            ...
            if not has_cell_normals:
                print(f"DIAG GII Collect: Finger {finger_idx_gii} has no valid cell normals.")
                continue
            for obj_pt_idx, obj_point in enumerate(object_points_global_static):
                try:
                    ...
                    if dist_gii < collision_threshold and dist_gii < min_dist_per_finger_final[finger_idx_gii]:
                        ...
                        # else: # Optional: print if normal candidate is bad
                            # print(f"DIAG GII Collect: Finger {finger_idx_gii}, ObjPt {obj_pt_idx}: Normal candidate invalid.")
                except Exception as e_gii_collect:
                    print(f"DIAG GII Collect: Exception for finger {finger_idx_gii}, ObjPt {obj_pt_idx}: {e_gii_collect}")
                    continue
    ...
    if len(valid_gii_contacts_final_state) < 2:
        print(f"DIAG: Exiting: Not enough valid GII contacts found after final collection: {len(valid_gii_contacts_final_state)}")
        return 0.0, final_contact_pressures, deformed_finger_meshes_at_contact, []
    if object_centroid_global_static is None:
        print("DIAG: Exiting: Object centroid is None before GII calculation.")
    ...
    if final_gii_value == 0.0 and gii is None:
        print(f"DIAG: calculate_gii_multi_contact returned None for r={r_value:.2f}, combo={finger_indices_tuple}, signs={normal_sign_multipliers}")
    elif final_gii_value == 0.0 and gii == 0.0:
         print(f"DIAG: calculate_gii_multi_contact returned 0.0 for r={r_value:.2f}, combo={finger_indices_tuple}, signs={normal_sign_multipliers}")
...
if __name__ == '__main__':
    ...
    object_target_center = np.array([0, 0, CUBOID_CENTER_OFFSET_Z])
    object_points_global_static, o3d_cuboid_mesh = create_cuboid_point_cloud(CUBOID_SIZE, object_target_center, CUBOID_INITIAL_ROTATION_EULER_ANGLES_DEG, n_points=CUBOID_N_POINTS)
    if object_points_global_static is None or object_points_global_static.shape[0] == 0: sys.exit("错误:未能生成长方体物体点云")
    pv_vertices = np.asarray(o3d_cuboid_mesh.vertices)
    pv_faces = np.hstack([[3] + face.tolist() for face in np.asarray(o3d_cuboid_mesh.triangles)])
    object_mesh_for_visualization = pv.PolyData(pv_vertices, faces=pv_faces)
    ...
    print(f"已生成长方体物体，目标中心: {object_target_center}, 计算质心: {object_centroid_global_static.round(2)}, 点数: {num_object_points_global_static}")
    ...
    if SHOW_ONE_TIME_INITIAL_CONFIG_PREVIEW and R_VALUES_TO_TEST.size > 0 and all_finger_combinations:
        ...
        if object_mesh_for_visualization: preview_plotter_one_time.add_mesh(object_mesh_for_visualization, color=object_point_color_viz, style='surface', opacity=0.7)
        else: preview_plotter_one_time.add_points(object_points_global_static, color=object_point_color_viz, point_size=3)
    ...
    DEBUG_NORMAL_SIGNS = False # Set to True to run the debug section for normal signs
    if DEBUG_NORMAL_SIGNS:
        print("\n--- 调试法向量符号组合 (交互式3D可视化) ---")
        debug_r_value = R_VALUES_TO_TEST[0] if len(R_VALUES_TO_TEST) > 0 else (max(CUBOID_SIZE[:2])/2.0 + finger_width * 0.5)
            ...
            for s1 in sign_options:
                for s2 in sign_options:
                    ...
                    if final_finger_meshes_debug and isinstance(final_finger_meshes_debug, list) and all(f is not None for f in final_finger_meshes_debug) and final_contacts_info_debug:
                        ...
                        if object_mesh_for_visualization: plotter_debug.add_mesh(object_mesh_for_visualization, color=object_point_color_viz, style='surface', opacity=0.3)
    ...
    for r_val_iter in R_VALUES_TO_TEST:
        ...
        for finger_combo_indices_iter in all_finger_combinations:
            ...
            if gii_value > 0:
                ...
                # To include 0s in average, uncomment below:
                # for i_finger_in_combo in range(len(finger_combo_indices_iter)):
                #    slot_idx_in_combo = finger_combo_indices_iter[i_finger_in_combo]
                #    if 0 <= slot_idx_in_combo < NUM_FINGER_SLOTS_CASE_STUDY:
                #         gii_lists_for_current_r[slot_idx_in_combo].append(gii_value)
    ...
    try:
        ...
        if 'object_mesh_for_visualization' in globals() and object_mesh_for_visualization is not None: del object_mesh_for_visualization; object_mesh_for_visualization = None
