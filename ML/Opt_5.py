...
sphere_translation = np.array([10, -5, sphere_radius + 25.0])
...
# 初始点寻找参数
INITIAL_R_FOR_SEARCH = tray_radius * 0.65
PRESSURE_STEP_INIT_SEARCH = 100.0
# 贝叶斯优化相关参数
N_CALLS_BO = 50
N_INITIAL_POINTS_BO = 10
R_BOUNDS = (tray_radius * 0.3, tray_radius * 0.9)
P_BOUNDS = (0.0, max_pressure)
# 可视化参数
contact_marker_radius = 1.0; contact_normal_length = 5.0; contact_plane_size = 4.0
show_finger_normals = True # 控制是否显示所有手指表面法向量
finger_normal_vis_scale = 1.0 # 手指表面所有法向量的显示长度
finger_normal_color = 'gray' # 手指表面所有法向量的颜色
contact_normal_color = 'black' # 接触点处法向量颜色
...
def calculate_gii_multi_contact(contacts_info, object_centroid, mu, eigenvalue_threshold):
    ...
    for contact in contacts_info:
        ...
        n_contact = - (normal_finger / norm_mag) # Normal pointing from object into finger
    ...
    try:
        ...
        if len(non_zero_eigenvalues) > 0:
            ...
            elif lambda_min < 0: return None # Issue
...
def evaluate_grasp(r, p1, p2, p3):
    ...
    mesh_generation_successful = True # Flag to track mesh generation
    ...
    for i in range(3):
        ...
        try:
             ...
             if final_transformed_mesh.n_cells > 0:
                 final_transformed_mesh.compute_normals(cell_normals=True, point_normals=False, inplace=True, auto_orient_normals=True, non_manifold_traversal=False)
        except Exception: mesh_generation_successful = False; break
        deformed_finger_meshes_world_this_eval.append(final_transformed_mesh)

    if not mesh_generation_successful:
        # --- START: 恢复/添加打印语句 ---
        print(" -> 网格生成失败, Cost=10.0") # (可选)
        # --- END: 恢复/添加打印语句 ---
        return 10.0
    ...
    if has_overlap_eval: grasp_state_eval = "Overlap"; cost = 5.0
        ...
        # else: No Contact, cost = 4.0

    # --- START: 恢复/添加打印语句 ---
    # 打印评估结果 (取消注释或修改)
    if grasp_state_eval == "Contact" and cost < 0:
        print(f" -> State={grasp_state_eval}, GII={-cost:.4f}, Cost={cost:.4f}")
    else:
        print(f" -> State={grasp_state_eval}, Cost={cost:.4f}")
    # --- END: 恢复/添加打印语句 ---

    return cost
# --- END of evaluate_grasp function ---
...
if __name__ == '__main__':
    ...
    # --- 步骤 1: 寻找初始接触点 ---
    initial_search_result = find_initial_grasp(
        initial_r=INITIAL_R_FOR_SEARCH,
        pressure_step=PRESSURE_STEP_INIT_SEARCH,
        max_pressure_init=max_pressure )

    # --- 步骤 1.5: 可视化初始搜索结果 ---
    if initial_search_result and 'finger_meshes' in initial_search_result:
        print("\n可视化初始搜索结束时的状态...")
        plotter_init_viz = pv.Plotter(window_size=[800, 600], title="Initial Grasp Search Result")
        ...
        init_meshes = initial_search_result['finger_meshes']
        ...
        init_point_status = initial_search_result.get('object_point_status', ['Gray'] * num_object_points_global_static)
        ...
        init_status_text = f"Initial Search Result: {initial_search_result.get('status', 'N/A')}\nr={initial_search_result.get('r', 'N/A'):.2f}, P={np.array(initial_search_result.get('pressures', [0,0,0])).round(0)}"
        plotter_init_viz.add_text(init_status_text, position="upper_edge", font_size=10)
        plotter_init_viz.camera_position = 'iso'
        print("显示初始搜索结果... 按 Q 关闭此窗口以继续。")
        plotter_init_viz.show()
    else: print("错误：初始点搜索未能返回有效结果或手指网格。"); sys.exit()

    # --- 步骤 2: 准备并执行贝叶斯优化 ---
    initial_points_for_bo = []
    proceed_to_bo = False
    if initial_search_result and initial_search_result['status'] == 'FoundInitialPoint':
        print(f"\n使用找到的初始有效接触点参数进行贝叶斯优化: r={initial_search_result['r']:.2f}, P={initial_search_result['pressures']}")
        initial_params = [initial_search_result['r']] + initial_search_result['pressures']
        initial_points_for_bo.append(initial_params)
        proceed_to_bo = True
    else: print(f"\n警告：初始搜索未能找到有效的三点接触状态 (状态: {initial_search_result.get('status', 'Unknown')})。跳过贝叶斯优化。")

    result = None
    if proceed_to_bo:
        if not initial_points_for_bo and N_INITIAL_POINTS_BO <= 0: print("错误：无初始点且不允许随机探索。"); sys.exit()
        print("\n开始贝叶斯优化...")
        try:
            current_x0 = initial_points_for_bo if initial_points_for_bo else None
            current_n_initial = N_INITIAL_POINTS_BO if not initial_points_for_bo else max(0, N_INITIAL_POINTS_BO - len(initial_points_for_bo))
            result = gp_minimize(
                func=evaluate_grasp, dimensions=dimensions, acq_func="EI",
                n_calls=N_CALLS_BO, n_initial_points=current_n_initial,
                x0=current_x0, random_state=123, noise=1e-6
             )
    ...
    if result is not None:
        ...
        # --- 步骤 4: 使用最优参数进行最终可视化 ---
        print("\n使用最优参数生成最终可视化...")
        final_r = best_params['r']; final_pressures = [best_params['p1'], best_params['p2'], best_params['p3']]
        final_finger_meshes = []; all_final_preds_valid = True
        dot_prod_tolerance = 1e-6
        ...
        if all_final_preds_valid and len(final_finger_meshes) == 3:
            ...
            final_finger_dot_products = [[] for _ in range(3)]
            final_has_overlap = False; final_num_contact_points = 0
            for obj_pt_idx, obj_point in enumerate(object_points_global_static):
                closest_dist_for_this_pt = float('inf'); finger_idx_for_this_pt = -1
                normal_for_this_pt = None; pt_on_mesh_for_this_pt = None
