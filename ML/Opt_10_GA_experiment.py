# -*- coding: utf-8 -*-
# 虚拟实验2：利用GraspNet预测的末端位姿与YCB数据集中的物体点云进行构型遗传算法优化并可视化
import numpy as np
...
import random # 用于遗传算法和随机采样
import itertools # 导入 itertools 用于生成排列组合
import vtk # For VTK font constants

# --- 打印版本信息 ---
try:
    print(f"Opt_10_GA_experiment.txt (已将贝叶斯优化替换为遗传算法)")
...
def visualize_failed_ga_step(r_param, chosen_indices_param, pressures_param, finger_meshes_param, failure_reason_str):
    """
    使用美化参数来可视化GA失败的步骤。
    """
    ...
    plotter_title = f"GA失败调试 #{FAILED_GRASP_VIS_COUNT}: {failure_reason_str}"
    ...
    print(f"\n(PyVista) 显示GA失败步骤 #{FAILED_GRASP_VIS_COUNT}。原因: {failure_reason_str}。请关闭窗口以继续...")
    ...
    try:
        ...
        while True:
            ...
            if current_pressure_iter > max_pressure_iterations:
                ...
                print(f"  GA 评估: {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print}")
                if DEBUG_VISUALIZE_FAILED_GRASPS:
                    visualize_failed_ga_step(r, chosen_indices, current_pressures, deformed_finger_meshes_at_contact, failure_reason_for_vis)
            ...
            for i in range(3):
                ...
                angle_rad_ga = np.radians(current_angle_deg)
                rot_angle_z_placing = angle_rad_ga + np.pi / 2.0
                rot_z_placing = create_rotation_matrix_z(rot_angle_z_placing)
                target_pos_on_circle = np.array([ r * np.cos(angle_rad_ga), r * np.sin(angle_rad_ga), 0.0 ])
            ...
            if object_points_global_static is not None and num_object_points_global_static > 0:
                ...
                if has_overlap_this_iter:
                    ...
                    print(f"  GA 评估: {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print}")
                    if DEBUG_VISUALIZE_FAILED_GRASPS:
                        visualize_failed_ga_step(r, chosen_indices, current_pressures, current_step_finger_meshes, failure_reason_for_vis)
            ...
            if all_fingers_established_this_iter_round:
                ...
                if len(final_gii_contacts_collected) >= 2:
                    ...
                    if gii is not None and gii > 1e-9:
                        ...
                        print(f"  GA 评估: {current_call_params_str} -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print:.4f}")
                        ...
                        print(f"  GA 评估: {current_call_params_str} ({failure_reason_for_vis}) -> 成本: {cost_to_return:.4f}, GII: {gii_value_to_print if isinstance(gii_value_to_print, str) else gii_value_to_print:.4f}")
                        if DEBUG_VISUALIZE_FAILED_GRASPS:
                             visualize_failed_ga_step(r, chosen_indices, return_pressures, return_meshes, failure_reason_for_vis)
                        return (cost_to_return, return_pressures, return_meshes, return_contacts)
                else:
                    cost_to_return = COST_LOW_GII_OR_FEW_CONTACTS
        ...
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(f"DEBUG_GA: EXCEPTION in evaluate_grasp for {current_call_params_str}: {e_eval_grasp}")
        traceback.print_exc()
        print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        cost_to_return = 20.0
...
if __name__ == '__main__':
    ...
    N_GENERATIONS = 4
    N_ELITES = 2
    MUTATION_RATE = 0.2  # 较高的突变率以增加多样性
    CROSSOVER_RATE = 0.8

    # --- 遗传算法辅助函数 ---
    def create_individual():
        """创建一个随机个体 (染色体)"""
        r = random.uniform(R_BOUNDS[0], R_BOUNDS[1])
        combo_idx = random.randint(0, num_valid_combinations - 1)
        return [r, combo_idx]

    def crossover(parent1, parent2):
        """对两个父代进行交叉操作"""
        if random.random() > CROSSOVER_RATE:
            return parent1[:], parent2[:] # 返回副本
        
        # r值采用算术交叉
        alpha = random.random()
        child1_r = parent1[0] * alpha + parent2[0] * (1 - alpha)
        child2_r = parent2[0] * alpha + parent1[0] * (1 - alpha)
        
        # combo_idx采用单点交叉 (此处只有一点,所以直接交换)
        child1_combo_idx = parent1[1]
        child2_combo_idx = parent2[1]
        if random.random() < 0.5:
            child1_combo_idx, child2_combo_idx = child2_combo_idx, child1_combo_idx
            
        return [child1_r, child1_combo_idx], [child2_r, child2_combo_idx]

    def mutate(individual):
        """对个体进行变异操作"""
        # r值变异: 在小范围内加减
        if random.random() < MUTATION_RATE:
            mutation_val = (random.random() - 0.5) * (R_BOUNDS[1] - R_BOUNDS[0]) * 0.1 # 变异范围为总范围的10%
            individual[0] += mutation_val
            individual[0] = np.clip(individual[0], R_BOUNDS[0], R_BOUNDS[1]) # 确保不出界
        # combo_idx变异: 随机选择一个新的
        if random.random() < MUTATION_RATE:
            individual[1] = random.randint(0, num_valid_combinations - 1)
        return individual
    ...
    # --- GA 主循环 ---
    print(f"\n开始遗传算法 (GA) 优化")
    print(f"种群数量: {POPULATION_SIZE}, 迭代次数: {N_GENERATIONS}, 精英保留数量: {N_ELITES}")
    ...
    for gen in range(N_GENERATIONS):
        ...
        for ind_idx, ind in enumerate(population):
            print(f"  评估个体 {ind_idx+1}/{POPULATION_SIZE}...")
            cost, _, _, _ = evaluate_grasp(
                r=ind[0],
                pos_idx1=all_valid_finger_combinations_global[int(ind[1])][0],
                pos_idx2=all_valid_finger_combinations_global[int(ind[1])][1],
                pos_idx3=all_valid_finger_combinations_global[int(ind[1])][2]
            )
            costs.append(cost)

        # 适应度是成本的负数（我们要最小化成本，即最大化适应度）
        # 为避免负适应度问题，对成本进行转换
        # 将成本值转换为适应度，确保所有适应度为正
        max_cost = max(costs) if costs else 0
        fitnesses = [(max_cost - c) + 1e-6 for c in costs]

        # 更新全局最优解
        min_cost_in_gen = min(costs)
        if min_cost_in_gen < best_cost_overall:
            best_cost_overall = min_cost_in_gen
            best_individual_overall = population[np.argmin(costs)][:] # 存储副本
            print(f"  新最优解! 成本: {best_cost_overall:.4f}, 参数: r={best_individual_overall[0]:.3f}, combo_idx={int(best_individual_overall[1])}")
        ...
        elites = [population[i][:] for i in sorted_indices[:N_ELITES]] # 存储副本
        next_population.extend(elites)
        print(f"  保留 {len(elites)} 个精英个体。本代最优成本: {min_cost_in_gen:.4f}")
    ...
    if best_individual_overall:
        best_p_list_ga = best_individual_overall
        best_c_ga = best_cost_overall

        best_r_ga = best_p_list_ga[0]
        best_combo_idx_ga = int(best_p_list_ga[1])
        ...
        final_cost_viz, final_pressures_viz, final_meshes_viz, final_contacts_viz = evaluate_grasp(
            r=best_r_ga,
            pos_idx1=best_pos_indices_ga[0],
            pos_idx2=best_pos_indices_ga[1],
            pos_idx3=best_pos_indices_ga[2]
        )
        DEBUG_VISUALIZE_FAILED_GRASPS = original_debug_vis_state

        best_gii_eval_ga = -final_cost_viz if final_cost_viz < -1e-9 else 0.
        ...
        print(f"优化找到成本 (GA best cost): {best_c_ga:.4f}")
        print(f"重评成本 (from full eval): {final_cost_viz:.4f}")
        if best_gii_eval_ga > 0: print(f"重评GII (derived from cost): {best_gii_eval_ga:.4f}")
        ...
        if final_meshes_viz and len(final_meshes_viz) == 3 and all(m is not None for m in final_meshes_viz):
            ...
            state_disp_ga=f"Cost={final_cost_viz:.3f}"; gii_disp_ga=f"GII={best_gii_eval_ga:.3f}" if best_gii_eval_ga>0 else "GII:N/A"

            # 使用新的美化绘图器
            plotter_final_pv = setup_publication_plotter("PyVista - Optimal Grasp (GA)", off_screen_default=False)
            ...
            for i_fv_ga,m_fv_ga in enumerate(final_meshes_viz):
                if m_fv_ga:
                    plotter_final_pv.add_mesh(m_fv_ga, color=finger_color_viz, style='surface', opacity=0.95, smooth_shading=True, show_edges=True, edge_color='dimgray', line_width=0.5, name=f'f_final_pv_{i_fv_ga}')
            ...
            plotter_final_pv.add_text(f"Optimal (GA):{params_txt_ga}\n{state_disp_ga}\n{gii_disp_ga}", position="upper_left", font=font_family_viz, font_size=10, color=text_color_viz)
            ...
            visualize_poses_with_open3d(
                ...
                window_title="Open3D - Optimal Poses & Axes (GA)"
            )
        else:
