...
cuboid_dims = np.array([25.0, 20.0, 30.0]) # Length(X), Width(Y), Height(Z)
num_cuboid_points = 5000
cuboid_rotation_angle_z = np.radians(0)
cuboid_center_translation = np.array([0, 0, cuboid_dims[2]/2.0 + 25.0])
...
# === START: 修改优化和初始搜索参数 ===
NUM_INITIAL_SEARCHES = 5 # <<< 初始搜索尝试次数
...
object_center_global_static = np.array([0.,0.,0.])
...
def evaluate_grasp(r, p1, p2, p3, angle_offset): # <<< 包含 angle_offset
    ...
    # (网格生成、碰撞检测、状态判断逻辑不变，内部使用 angle_offset)
    for i in range(3):
...
def find_initial_grasp(initial_r, initial_angle_offset, pressure_step, max_pressure_init): # <<< 添加 initial_angle_offset
    ...
    while True:
        ...
        # --- 1. 生成手指网格 (使用 initial_angle_offset) ---
        ...
        for i in range(3):
            ...
            # --- 使用传入的 initial_angle_offset ---
            base_angle_deg = [0, 120, 240][i]
            current_angle_deg = base_angle_deg + initial_angle_offset # <<< 使用参数
            ...
            target_pos_on_circle = np.array([ initial_r * np.cos(angle_rad), initial_r * np.sin(angle_rad), 0.0 ]) # <<< 使用 initial_r
        ...
        # (后续逻辑与 evaluate_grasp 中的碰撞检测和状态判断类似，但返回字典)
        ...
        if has_overlap_init:
            print("  初始点搜索: 检测到重叠。");
            return {**return_dict_base, 'status': 'Overlap'}
        ...
        if any(finger_intersects_init):
            print("  初始点搜索: 检测到交叉穿透。");
            return {**return_dict_base, 'status': 'Intersection'}
        for i in range(3):
        ...
        if all(finger_contact_achieved_init):
            print(f"--- 成功找到初始接触点组合 (r={initial_r:.2f}, offset={initial_angle_offset:.1f}°, P={current_pressures_init.round(0)}) ---") # <<< 修改打印信息
...
if __name__ == '__main__':
    ...
    if OBJECT_TYPE == 'cuboid':
        ...
        object_center_local = np.array([0., 0., 0.])
    else: print(f"错误: 未知的 OBJECT_TYPE '{OBJECT_TYPE}'"); sys.exit()
    ...
    if object_points_global_static is None or object_points_global_static.shape[0] == 0: print("错误：无法生成或变换物体点云。"); sys.exit()
    ...
    if OBJECT_TYPE == 'cuboid': object_mesh_global_static = pv.Cube(center=object_center_global_static, x_length=cuboid_dims[0], y_length=cuboid_dims[1], z_length=cuboid_dims[2]).rotate_z(np.degrees(cuboid_rotation_angle_z), inplace=False)
    elif OBJECT_TYPE == 'sphere': object_mesh_global_static = pv.Sphere(center=object_center_global_static, radius=sphere_radius)
    ...
    collected_initial_points = [] # 存储找到的有效初始点 [r, p1, p2, p3, angle_offset]
    last_search_attempt_result = None # 存储最后一次尝试的结果，用于可视化

    # 定义随机半径和角度的搜索范围
    if OBJECT_TYPE == 'cuboid':
        cuboid_max_xy_radius = 0.5 * np.sqrt(cuboid_dims[0]**2 + cuboid_dims[1]**2)
        r_search_min = cuboid_max_xy_radius * 1.1
    elif OBJECT_TYPE == 'sphere':
        r_search_min = sphere_radius * 1.1
    else: r_search_min = R_BOUNDS[0]
    ...
    print(f"搜索范围: r in [{r_search_min:.2f}, {r_search_max:.2f}], angle_offset in [{angle_offset_search_min:.1f}, {angle_offset_search_max:.1f}]")

    for k in range(NUM_INITIAL_SEARCHES):
        print(f"\n--- 初始搜索尝试 {k+1}/{NUM_INITIAL_SEARCHES} ---")
        # 随机生成 r 和 angle_offset
        ...
        if search_result and search_result['status'] == 'FoundInitialPoint':
            ...
            if in_bounds:
                 collected_initial_points.append(params_5d) # <<< 存储 5D 参数
    ...
    if last_search_attempt_result and 'finger_meshes' in last_search_attempt_result:
        print("\n可视化最后一次初始搜索尝试的状态...")
        plotter_init_viz = pv.Plotter(window_size=[800, 600], title=f"Initial Search Attempt {NUM_INITIAL_SEARCHES}/{NUM_INITIAL_SEARCHES}") # 更新标题
    ...
    if proceed_to_bo:
        ...
        try:
            result = gp_minimize(
                func=evaluate_grasp,            # 5D 目标函数
                dimensions=dimensions,          # 5D 维度定义
                acq_func="EI",
                n_calls=N_CALLS_BO,
                n_initial_points=num_random_points_needed, # BO 额外生成的随机点数
                x0=collected_initial_points if collected_initial_points else None, # 提供的 5D 初始点列表
                random_state=123,
                noise=1e-6
             )
    ...
    if result is not None:
        ...
        best_params = dict(zip(param_names, best_params_list)) # param_names 已包含 angle_offset
